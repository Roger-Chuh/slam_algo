function DTD(obj, imgInd, imgPrv, imgCur, depthPrv, depthCur, thetaList)
global ReplaceOnlyP2 ReplaceOnlyP2Z thetaListGT ShiftByCoord probPath mean_x_err...
    err_x rou rou_temp_list
...
err = [0 0; er];alpha = rou;figure,subplot(1,2,1);plot(diff(err_x));grid on;hold on;plot(diff(alpha.*err(:,2)));subplot(1,2,2);plot(diff(err_x) ./ diff(alpha.*err(:,2)));grid on;title(num2str(mean(diff(err_x) - diff(alpha.*err(:,2)))));
if 0
    traceInfoMat1 = obj.traceInfoMat;
    traceManager1 = obj.traceManager;
    %     combData
...
global newRender ShiftLK newShift ReplaceOnlyP2 ReplaceOnlyP2Z ShiftByCoord doubleShift mean_x_err...
    rou shiftPixThr doubleShift2 err_x switchMethod rou_temp_list
...
rou_temp = abs(mean(cur_lk_gt2(:,1) - cur_lk_gt2(:,5))/rad2deg(p2cBodyRotAng_2));
...
%             angBias_ = -deg2rad(err_x(end,1)/rou);
            angBias_ = -1*deg2rad(err_x(end,1)/4);
%             angBias_ = -deg2rad(mean_x_err(end,1)/rou);
            ...
            [tempCurPt_2_newShift_222] = VisualLocalizer.GetGtTrace2(b2cPmat, double(p2cBodyRotAng_22_onlyP2), [cur_lk_gt2(:,5:6)],cur_lk_gt2(:,7),intrMat);
            
%             rou_temp = abs(mean(tempCurPt_2_newShift_222(:,1) - cur_lk_gt2(:,5))/rad2deg(p2cBodyRotAng_22_onlyP2));
            

            X_shift_all2_0 = tempCurPt_2_newShift_2 - cur_lk_gt2(:,1:2);
            X_shift_all2 = tempCurPt_2_newShift_222 - cur_lk_gt2(:,1:2);
            
            if switchMethod
                err_x = [err_x; err_x(end) + mean(X_shift_all2(:,1))];
            else
                err_x = [err_x; err_x(end) + mean(X_shift_all(:,1))];
                tempCurPt_2_newShift_2 = TempCurPt_2;
            end
            ...
            if ~switchMethod
                ...
                %                 p2cBodyRotAng_22_onlyP2 = p2cBodyRotAng_22_onlyP2 - sign(p2cBodyRotAng_22_onlyP2)*deg2rad(mean(X_shift_all2(:,1))/rou_temp);
                %                 p2cBodyRotAng_22_onlyP2 = p2cBodyRotAng_22_onlyP2 - sign(p2cBodyRotAng_22_onlyP2)*deg2rad(mean(X_shift_all2(:,1))/rou);
                p2cBodyRotAng_22_onlyP2 = p2cBodyRotAng_22_onlyP2 - deg2rad(mean(X_shift_all2(:,1))/rou);
            end
            
            [tempCurPt_2_newShift_222_new] = VisualLocalizer.GetGtTrace2(b2cPmat, double(p2cBodyRotAng_22_onlyP2), [cur_lk_gt2(:,5:6)],cur_lk_gt2(:,7),intrMat);
            
            X_shift_all_new = tempCurPt_2_newShift_222_new - cur_lk_gt2(:,1:2);
            
            if 0
                err_x(end,1) = err_x(end-1,1) + mean(X_shift_all_new(:,1));
            end
            rou_temp_list = [rou_temp_list; rou_temp];
            %                         p2cBodyRotAng_22_onlyP2 = p2cBodyRotAng_2_onlyP2 - deg2rad(err_x(end,1)/rou);
            p2cBodyRotAng_22_onlyP2_0 = p2cBodyRotAng_22_onlyP2;
%             p2cBodyRotAng_22_onlyP2 = p2cBodyRotAng_22_onlyP2 - deg2rad(err_x(end,1)/rou);
%             p2cBodyRotAng_22_onlyP2 = p2cBodyRotAng_2_onlyP2 - deg2rad(err_x(end,1)/rou);
%             p2cBodyRotAng_22_onlyP2 = p2cBodyRotAng_2_onlyP2 - deg2rad((err_x(end,1) - err_x(end-1,1))/rou);
...
numThr = 30;
for i = 1 : size(traceInfoMat,1) - 20 % 45 % size(traceInfoMat,1) - 2
