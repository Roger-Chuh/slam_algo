...
global OUTPUT_ROOT RECORDED_BIN_ROOT DUMP_ROOT RESULT_ROOT DRAWPOLYGON ANGLEONLY SHOWPOLYGON USEGOLDENDISP USERIGHT UPDATEDEPTH DEPTHITERNUM probPath ...
    ...
    CUTTHETAANDFORCEPLATFORM DIFFTHETATHR GOLDENZ CUTTHETAANDFORCEPLATFORM_NODIFF FORCEEXPEVEN EXPZERO GTTRACKING SOFTP2
    ...
    for j = 1 : size(curKeySeqData.KeySeq.imgHeader,1)-1
        if needReplace
            if keySeqNum > 1 && j ==1
                ...
                vsl.configParam.disparity_error = 2; 1.5; 2;   2; 1;
                ...
                vsl.configParam.theta_sample_step =  deg2rad(0.05);
                ...
                vsl.configParam.disparity_sigma = 3; 2; 3; 1; %  1/3 1 3
                vsl.configParam.disparity_beta = 100;10; 10; 100; %  1/3 10 100
                
                
                vsl.configParam.reproj_sigma = 1; 1.5; 2; 3; 1.5; 2; 1; 1.5; % 1/3
                vsl.configParam.reproj_beta = 100; % 1/3
                vsl.configParam.reproj_sigma_right = 1; 1.5; 2; 3 ;1.5; 2; 1; 1.5; % 1/3
                ...
                vsl.configParam.theta_diff_margin = 0.005; 0.01; 0.005;  0.005; 0.1; 0.08; 0.3; 0.2;0.003; 0.3; 0.05; 0.003; 0.01; 0.01; 0.005; 0.01; 0.005;
                
                vsl.configParam.expectation_theta_radius = deg2rad(0.2);% 0.5
                vsl.configParam.expectation_theta_expand_num = 20; 10; 20;  10;30;
        ...
        figure(10),clf;plot(angErr,'r');hold on;plot(angErr2,'g');plot(angErr22,'c');plot(angErr3,'b');plot(find(vsl.keyFrameFlagList),angErr2(find(vsl.keyFrameFlagList)),'om');plot(pulse,'k');legend('pnp err','opt err','opt pnp err','upper err', 'lower err','key frame','one cycle');title(num2str(vsl.configParam.theta_percentage));
        try
            plot(angErrGT(1:length(angErr)),'-k');
        catch
            dbdabaa = 1;
        end
        ...
        try
            save(fullfile(probPath,'vsl.mat'), 'vsl');
        catch
            gssagq = 1;
        end
