...
function [newpts, T] = normalise2dpts(pts)
    ...
    newp(2,finiteind) = pts(2,finiteind)-c(2);%   把所有点的x、y坐标减掉x、y的平均值
    
    dist = sqrt(newp(1,finiteind).^2 + newp(2,finiteind).^2);
    meandist = mean(dist(:));  % Ensure dist is a column vector for Octave 3.0.1求各个新点到新原点的距离
    
    scale = sqrt(2)/meandist;%求平均距离和根号2之间的比例尺度
