classdef VisualLocalizer_Bak_20191031 < BaseLocalizer
    ...
    methods
        ...
        function Localize(obj, imgL, imgR, refMotionState, refRobotPoseWcs, frameInd)
            ...
            if 0  %size(obj.poseWcsList,1) == 97; 32; 81; 257
                doInsert = 2;
            end
        ...
        function [robotPoseWcs] = PureRotationTrackAndLocalize(obj)
            global  DRAWPOLYGON ANGLEONLY SHOWPOLYGON USEGOLDENDISP NEWTRACKER
            %             intrMat = Get(obj.camModel, 'PinholeIntrMat', 1,1);
...
%                         if 1
                            ...
                            if DRAWPOLYGON
                                if ~isempty(inlierId)
                                    ...
                                    if 1
                                        try
                                            [angOpt1, inlierIdNew,  angRng1, pixKey ,pt2dCur, DispRefine, angOpt33] = RefineZ(obj, k2cRef,k2cBodyRotAng, inlierId,inlierId2, theta, DepthProbility,  DepthId,  AngleModalOrg,  AngleProbility,prevFeatPtList0,ptCcsZ0);  %, keyCcsXYZAll);
...
s.configParam.angleOrginRange = rad2deg(k2cRef) + s.configParam.angleRange;
                                ...
                                if ONLYP2
                                    ...
                                    if 1
                                        ...
                                        plot(rad2deg(angOpt3 - k2cRef), max(thetaProb),'or');
...
%                                         saveas(gcf,fullfile(probPath,sprintf(strcat('cutThetaDiff2_',probDir(end-14:end),'____',num2str(sum(obj.keyFrameFlagList)),'__%05d.fig'),length(dir(fullfile(probPath,strcat('cutThetaDiff2_',probDir(end-14:end),'____',num2str(sum(obj.keyFrameFlagList)),'__*.fig'))))+1)));
                                    ...
                                    if UPDATETracking_2
                                        ...
                                        if sum(thetaProbUse < obj.configParam.only_p2_prob_cutoff_threshold) > 0
                                            thetaProbUse_(thetaProbUse > obj.configParam.only_p2_prob_cutoff_threshold) = max(thetaProbUse(thetaProbUse < obj.configParam.only_p2_prob_cutoff_threshold));
                                        else
                                           thetaProbUse_ = ones(length(thetaProbUse_),1); 
                                        end
                                        thetaProbUse__ = thetaProbUse_./sum(thetaProbUse_);
                                        
                                        [curPredPtIcs,inTrackFlag,curPredPtIcs0,inTrackFlag0] = NewTracking(obj, size(obj.featPtManager.localTrace.ptIcsX,1),prevFeatPtList0,ptCcsZ0,intrMat, obj.currRefRobotRotAngWcs - obj.keyRefRobotRotAngWcs, AngleModalOrg, obj.keyFrameDepth, obj.prvDepthVisual, obj.depthVisual,DepthProbility, DepthId, thetaProbUse__,1);
                                ...
                                if 1 % ~ONLYP2
                                    if 1
                                        ...
                                        [thetaProb,idM,angOpt, ReprojErrVecTmp, ReprojErrVecRTmp, ReprojErrVecTmpMatT, ReprojErrVecRTmpMatT,ValidFeatFlag,ValidFeatFlag0,ValidFeatFlagFusion,ValidFeatFlagQuant,ProbReprojVecAll, ProbReprojVecRAll, PixKeyVecTmp, PixKeyVecRTmp, Probb, ...
                                            ...
                                            [~,~,~, ReprojErrVecTmp, ReprojErrVecRTmp, ReprojErrVecTmpMatT, ReprojErrVecRTmpMatT,ValidFeatFlag,ValidFeatFlag0,ValidFeatFlagFusion,ValidFeatFlagQuant,ProbReprojVecAll, ProbReprojVecRAll, PixKeyVecTmp, PixKeyVecRTmp, Probb, ...
                                                validFeatFlagFusion,validFeatFlag, ProbReprojVec, ProbReprojVecR,bb,b,pixKeyVecTmpXCoordMatStack,pixKeyVecTmpYCoordMatStack,pixKeyVecRTmpXCoordMatStack,pixKeyVecRTmpYCoordMatStack]...
                                                = ReplayTheta(obj, thetaRng, b2c, keyCcsXYZAll, intrMat, Pix, disparityRng, baseline, ZZVec1, L2R, pt2dCur, pt2dCurR,ProbZ, ProbZTmp_norm, imgCur, imgCurR,thetaSamp);
                                            if 0
...
%                                     saveas(gcf,fullfile(probPath,sprintf(strcat('cutThetaDiff2_',probDir(end-14:end),'____',num2str(sum(obj.keyFrameFlagList)),'__%05d.fig'),length(dir(fullfile(probPath,strcat('cutThetaDiff2_',probDir(end-14:end),'____',num2str(sum(obj.keyFrameFlagList)),'__*.fig'))))+1)));
                                ...
                                obj.twoOnlyP2List = [obj.twoOnlyP2List; [rad2deg([angOpt3 angOpt4] - [k2cRef k2cRef])] ];
...
%                                     plot(rad2deg([optAng(1:end);thetaRng(idM)]) - rad2deg(obj.refAngList),'-xk');
                            ...
                            try
                                ...
                                qwa = checkId1111(1);figure(17),clf;subplot(2,1,1);plot([ProbZ(qwa,:)./max(ProbZ(qwa,:)); updatedProbZSum0(qwa,:)./max(updatedProbZSum0(qwa,:)); updatedProbZSum(qwa,:)./max(updatedProbZSum(qwa,:))]');hold on;plot(depthGTInd_update(qwa,1),updatedProbZSum(qwa,depthGTInd_update(qwa,1))./max(updatedProbZSum(qwa,:)),'*r');legend('orig Z','1st upadte','2nd update','gt');
                                qwa = checkId1111(end);             subplot(2,1,2);plot([ProbZ(qwa,:)./max(ProbZ(qwa,:)); updatedProbZSum0(qwa,:)./max(updatedProbZSum0(qwa,:)); updatedProbZSum(qwa,:)./max(updatedProbZSum(qwa,:))]');hold on;plot(depthGTInd_update(qwa,1),updatedProbZSum(qwa,depthGTInd_update(qwa,1))./max(updatedProbZSum(qwa,:)),'*r');legend('orig Z','1st upadte','2nd update','gt');
...
%                             doProb = 1; idM_Vec = -20:20; debugProb;
    ...
    methods
        function [curPredPtIcs,inTraceFlag,curPredPtIcs0,inTraceFlag0] = NewTracking(obj,keyFeatNum,prevFeatPtList,ptCcsZ,intrMat,k2cRef,angleModalOrg, keyDepthV, prevDepthV, currDepthV,DepthProbility, DepthId, AngleProbility,reCalc)
