function [angOpt, inlierId, angRng, PixUse, pt2dCurUse, DispRefineUse_, angOpt3, inFlag,thetaRngOut,thetaProbOut,angOptP2C, angOptK2C_, p2cOnlyP2_111,...
    ...
    meanErrAndStd,...
    p2cErrList,...
    objPervPtCcsZGolden1] = RefineZ_StandAlone_bak(k2cRef_00, obj, k2cRef,k2cPnp, inlierId0, inlierId2, theta, DepthProbility,  DepthId,  AngleModalOrg,  AngleProbility,prevFeatPtList0,ptCcsZ0,DispRefineUse__,NewDispRng,newDispStep,ProbZTmpReSamp,k2cBody,...
    keyFrameLen ,...
    disparity_resample_prob_threshold,...
    ThetaNoise ,...
    KeyFrameFlagList,...
...
if USEGOLDENDISP
    ...
    if keyFrameLen == 2
        ...
        dispGTTmp = dispMatGT(validInd);
        figure(FigBase + FigBase + 38),clf;subplot(1,3,1);hold on;plot(dispList(inlierId0) - dispGTTmp(inlierId0));title('stereoDisp - gtDisp');subplot(1,3,2);imshow(imgCur);hold on;
        ...
        if 1
            if isempty(LocalTrace.probZ)
                ...
                if 0
                    ...
                    LocalTrace.ptCcsZ(inlierId) = (intrMat(1,1).*norm(CamModel.transVec1To2))./(dispList(inlierId) + (princpPtR(1) - princpPtL(1)));
                    zList(inlierId) = LocalTrace.ptCcsZ(inlierId);
                    dispPrvList_(inlierId) = DispKeyInPrvRng(:,(size(DispRng,2)+1)/2);
                    depthPrvList_(inlierId) = (intrMat(1,1).*norm(CamModel.transVec1To2))./(dispPrvList_(inlierId) + (princpPtR(1) - princpPtL(1)));
                end
                ...
                for jk = 1 : length(inlierId)
                    [~, ZZVec1(jk,:)] = probDensity(dispList(inlierId(jk),:), ConfigParam.disparity_sigma,ConfigParam.disparity_beta,ConfigParam.reproj_beta, DispRng(jk,:),ConfigParam.disparity_sample_interval, 'disparity');
                end
            ...
            if 1 % ~USEGOLDENDISP %keyFrameLen == 2
                %                     figure(FigBase + FigBase + 38),clf;plot(dispList - dispMatGT(validInd));title('stereoDisp - gtDisp');
                %                 dispGTTmp = dispMatGT(validInd);
                figure(FigBase + 38),clf;subplot(1,3,1);hold on;plot(dispList(inlierId) - dispGTTmp(inlierId));title('stereoDisp - gtDisp');subplot(1,3,2);imshow(imgCur);hold on; %subplot(1,3,3);plot(dispErrCur);title('stereoDispCur - gtDispCur');
                disparityError = dispList(inlierId) - dispGTTmp(inlierId);
...
%                         k2cBodyPmat = PointCoordTransformer(BaseLocalizer.RotMatFromAngle(k2cRef0),zeros(3,1));
            ...
            if ~GTTRACKING
                ...
                if 1
                    ...
                    pt2dCur_0 = [LocalTrace.ptIcsX(inlierId,keyLength) LocalTrace.ptIcsY(inlierId,keyLength)];
            ...
            for jkl = 1 : DEPTHITERNUM
                ...
                if keyFrameLen > 2
                    ...
                    for jkj = 1 : length(inlierId)
                        [~, ZZVecPrv1(jkj,:)] = probDensity(dispPrvList_(inlierId(jkj),:), ConfigParam.disparity_sigma,ConfigParam.disparity_beta,ConfigParam.reproj_beta, DispRngP2C(jkj,:),ConfigParam.disparity_sample_interval, 'disparity');
                    end
