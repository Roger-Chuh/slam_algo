function [sampledDispRngPrvInKey, ProbZTmpReSamp, DispKeyInPrvRng, dispPInK_intersect2view, dispPInK] = IntersectCone(obj, inlierId, inlierIdPrv, imgKeyL, imgPrvL, b2c,DispRng, dispPrvList0, DepthProbility, DepthId,ConfigParam,KeyFrameFlagList,thetaRngMat,thetaProbMat,LocalTrace)
...
pixPrv = [LocalTrace.ptIcsX(:,end-1) LocalTrace.ptIcsY(:,end-1)];
dispPrvList = dispPrvList0(:);
...
for i = 1 : 1% length(prvThetaProbUse)
    ...
    ptKey_norm = intrMat\HomoCoord(pixKey',1);
    ...
    [XKey, errKey3,errPrv3] = GetBoundaryDepth(intrMat, k2pCam,ptKey_norm,metricPrevPtCcs, pixKey, pixPrv);
    ...
    if 1
        ...
        [XYZPrvInKeyRng] = GetXYZFromDepth(intrMat, repmat(pixKey,size(DispRng,2),1),ZPrv2KeyRng(:,(size(ZPrv2KeyRng,2) + 1)/2));
    end
    
%     dispPInK = (intrMat(1,1).*norm(obj.camModel.transVec1To2))./(XYZPrvInKeyRng(:,3) + (princpPtR(1) - princpPtL(1)));
    dispPInK = (intrMat(1,1).*norm(obj.camModel.transVec1To2))./(ZPrv2KeyRng(:,(size(ZPrv2KeyRng,2) + 1)/2) + (princpPtR(1) - princpPtL(1)));
    
    dispPInK_intersect2view = (intrMat(1,1).*norm(obj.camModel.transVec1To2))./(XKey(:,3) + (princpPtR(1) - princpPtL(1)));
    ...
    if 0
        figure,plot((ZPrv2KeyRng(:) - ZPrvInKeyRng(:)))
    end
...
if isempty(Prv2KeyBelong2ConeLower)
    ...
    pt1Id = find(ismember(round(X1(:,3),4),round(chosenConeZ,4)));
    pt2Id = find(ismember(X2(:,3),chosenConeZ));
    
    pt11Id = find(ismember(round(chosenConeZ,4),round(X1(:,3),4)));
