...
if ~isfield(renderparams,'useMex')
    renderparams.useMex = false; true;
end
...
if 0
    id1 = find(Vcam(:,3) > 0);
    id = find(Vcam(:,3) <= 0);
    flag1 = find(ismember(F(:,1), id));
    flag2 = find(ismember(F(:,2), id));
    flag3 = find(ismember(F(:,3), id));
    FF = F;
    FF(unique([flag1;flag2;flag3]),:) = [];
    
    idMat = [id1 [1:length(id1)]'];
    
    [DD] = pdist2(FF(:,1),idMat(:,1),'euclidean');
    ...
    [timeL__1, timeD__1, samePtFlagL1, samePtFlagD1] = KNN( Vxy, Vxy(FF(:,1),:));
    [timeL__2, timeD__2, samePtFlagL2, samePtFlagD2] = KNN(FF(:,2), idMat(:,1));
    [timeL__3, timeD__3, samePtFlagL3, samePtFlagD3] = KNN(FF(:,3), idMat(:,1));
end
    ...
    switch renderparams.lightingModel
        case 'localPointSource'
            ...
            if 0
                ...
                lightparams.f = cameraparams.f; lightparams.cx = cameraparams.cx; lightparams.cy = cameraparams.cy;
            end
...
switch renderparams.textureMode
    ...
    case 'usePerMeshColour'
        ...
        if 0
            colorCoeff = rand(size(texbuffer));
        else
            colorCoeff = ones(size(texbuffer));
            colorCoeff(220,237,:) = 0;
        end
        for c=1:3
            if 0
...
%                 colorCoeff = rand(3,1);
                texbuffer(:,:,c) = colorCoeff(:,:,c); renderparams.permeshcolour(c);
            end
        end
        texbuffer(mask) = NaN;
    otherwise
