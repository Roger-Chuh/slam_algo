function [XErrMat_body, YErrMat_body, thetaList, traceInfoMat,XErrMat_body_stack, YErrMat_body_stack, Img, validCnt,xErrMat,yErrMat,xErrMatFull,yErrMatFull] = CalibB2C66(inputDir, startt, endd, doDiff)
...
try
    dr = 0;
    traceManager = traceInfoMat{1,8};
    b2cPmat = traceInfoMat{1,9};
    for k = 1 : length(thetaList) - 1
        thetaP2C = thetaList(k+1) - thetaList(k);
        pixPrv0 = [traceManager.X(:,k) traceManager.Y(:,k)];
        pixCur0 = [traceManager.X(:,k+1) traceManager.Y(:,k+1)];
        pixCurLK0 = [traceManager.XLK(:,k+1) traceManager.YLK(:,k+1)];
        zPrvGT0 = traceManager.ZGT(:,k);
        zPrv0 = traceManager.Z(:,k);
        depthPrvGT = depthGT(:,:,k);
        depthPrvStereo = depthStereo(:,:,k);
        validPt = find(pixPrv0(:,1) > 0 & pixCur0(:,1) > 0);
        pixPrv = pixPrv0(validPt,:);
        pixCur = pixCur0(validPt,:);
        pixCurLK = pixCurLK0(validPt,:);
        
        
        zPrvGT = zPrvGT0(validPt,:);
        indPrv = sub2ind(size(depthPrvGT), round(pixPrv(:,2)), round(pixPrv(:,1)));
        zPrvGT_check = depthPrvGT(indPrv) - zPrvGT;
        assert(max(abs(zPrvGT_check)) == 0);
        
        zPrv = zPrv0(validPt,:);
        indPrv = sub2ind(size(depthPrvStereo), round(pixPrv(:,2)), round(pixPrv(:,1)));
        zPrv_check = depthPrvStereo(indPrv) - zPrv;
        assert(max(abs(zPrv_check)) == 0);
        
        
        [xyzPrvGT] = GetXYZFromDepth(intrMat, pixPrv,zPrvGT);
        RTTemp_comp = b2c * [roty(rad2deg(double(thetaP2C))) [0 0 0]';0 0 0 1] * inv(b2c);
        ptIcsTemp_gt = TransformAndProject(xyzPrvGT, intrMat, RTTemp_comp(1:3,1:3), RTTemp_comp(1:3,4));
        [pixGTCur] = VisualLocalizer.GetGtTrace2(b2cPmat, thetaP2C, [pixPrv],zPrvGT,intrMat);
        if 0
            figure,plot(pixGTCur - ptIcsTemp_gt)
        end
        
        
        
        ang = OnlyP2_ang(intrMat, pixPrv, pixCur, r_cam,tx, ty, tz);
        [pixCurShift] = VisualLocalizer.GetGtTrace2(b2cPmat, ang, [pixPrv],zPrv,intrMat);
%         pixCurShift = 
        OnlyP2Err(k,1) = rad2deg(ang - thetaP2C);
        TrackingErr{k,1} = [pixCurLK - pixGTCur pixCurShift - pixGTCur];
        TrackingErr{k,2} = [mean(pixCurLK - pixGTCur) mean(pixCurShift - pixGTCur)];
        TrackingErr{k,3} = [pixCur - pixGTCur pixCurShift - pixGTCur];
        TrackingErr{k,4} = [(pixCur - pixGTCur) - (pixCurShift - pixGTCur)];
        if dr
            figure,plot(TrackingErr{k,1}(:,1), TrackingErr{k,1}(:,2),'+r');hold on;plot(TrackingErr{k,1}(:,3), TrackingErr{k,1}(:,4), '.b');axis equal;title(sprintf('p2cErr: %05f',OnlyP2Err(k,1)));
        end
    end
    
    trackingError = cell2mat(TrackingErr(:,2));
    trackingErrorX = trackingError(:,3) - trackingError(:,1);
    angX = [OnlyP2Err trackingErrorX];
    
    figure,subplot(1,3,1);plot(angX(:,2)./angX(:,1));grid on;
    subplot(1,3,2);plot(angX);legend('ang err','x err');grid on;
    subplot(1,3,3);hist(angX(:,2)./angX(:,1),100);grid on;
catch
...
goodId = find(max(abs(x_p2c)') < 10.1);
...
try
    if ~doDiff
        hold on;plot(rad2deg(thetaList(1:end-1)), 48.*rad2deg(traceInfoMat{1,6}(2:end-1) - thetaList(2:end)));
    else
        %         hold on;plot(rad2deg(thetaList), 1.*rad2deg(traceInfoMat{1,6}(1:end-1) - thetaList));
        hold on;plot(rad2deg(thetaList(1:end-1)), 1.*rad2deg(traceInfoMat{1,6}(2:end-1) - thetaList(2:end)));
    end
    %     hold on;plot(rad2deg(thetaList), 1.*rad2deg(traceInfoMat{1,7}(1:end-1,:) - thetaList));
    %     hold on;plot(rad2deg(thetaList), 1.*rad2deg(traceInfoMat{1,7}(1:end-1,end) - thetaList));
    %     hold on;plot(rad2deg(thetaList), 1.*rad2deg(traceInfoMat{1,7}(1:end-1,2) - thetaList));
catch
    try
