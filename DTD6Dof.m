...
function DTD6Dof(obj, imgInd, imgPrv, imgCur, depthPrv, depthCur, thetaList, depthPrvGT123, depthCurGT123)
global probPath ReplaceOnlyP2 pixLenThr ShiftLK vslMat vslMatPNP num_levels deOutlierThr yMatPrv yMatCur yRng zRng initial_sigma default_dof para
pixLenThr = -1; 0.2;0; 0.2; 2; 0;
% num_levels = 2; 1; 2; 3;   3; 5; 3; 5; 3; 5;
...
%     [k2cBodyRotAng_2,Err_2,inId, reprojErrList] = VisualLocalizer.RotationAngleEstimate_PNP_(pnp_ang_est_max_margin,intrMat,pixPrv(inTrackFlag,:),predPtList(inTrackFlag,:),zList(inTrackFlag), find(inTrackFlag),true(sum(inTrackFlag),1),b2c,double(thetaP2C));
    ...
    inTrackId = find(trackingEr < pnp_ang_est_max_margin(3));
    inId_bak = inId;
    inId = inId(inTrackId);

    if 0
        figure,plot(c2cErr(:,1), c2cErr(:,2), '+r');axis equal
        figure,subplot(1,2,1),showMatchedFeatures(imgPrv, imgCur, pixPrv(inId_bak,:), predPtList(inId_bak,:));subplot(1,2,2),showMatchedFeatures(imgPrv, imgCur, pixPrv(inId,:), predPtList(inId,:))
    ...
    obj.traceManager.X(inId, 1) = pixPrvValid(inTrackId,1);
    obj.traceManager.Y(inId, 1) = pixPrvValid(inTrackId,2);
    obj.traceManager.Z(inId, 1) = zListValid(inTrackId,1);
    ...
    obj.traceManager.X(inId, 2) = pixCurValid(inTrackId,1);
    obj.traceManager.Y(inId, 2) = pixCurValid(inTrackId,2);
    obj.traceManager.Z(inId, 2) = vldPtCurValid(inTrackId,1);
    ...
    featPtList___1 = VisualLocalizer.DetectFast(imgPrv, 0.02);
...
if 0
    ...
    if inLierRate > inLierRateThr
        ...
        [rtTemp_all, outlierId_all] = posest(double(curPt(validPnP_all,:)), double(XYZ_Prv_all(validPnP_all,:)), inRa, intrMat, 'repr_err');
    end
...
%     [~,~,inId] = VisualLocalizer.RotationAngleEstimate_PNP_(pnp_ang_est_max_margin,intrMat,prvPt,curPt,zList, id,true(length(id),1),b2c,double(p2cRef));
    pnp_ang_est_max_margin_2 = [deg2rad([-0.2 0.2]) deOutlierThr];
    
    if 1 % ShiftLK && newShift
        pnp_ang_est_max_margin_2 = [deg2rad([-0.01 0.01]) deOutlierThr];
    end
    
    if 1 % 20210719
        pnp_ang_est_max_margin_2 = [deg2rad([-0.5 0.5]) deOutlierThr];
    end
    if 1 % ShiftLK && newShift
        if obj.switchDepth
...
[inIdKey, vslPoseVec] = PnPDeoutlier(featIdKey,[nearestKeyX(featIdKey, 1) nearestKeyY(featIdKey, 1)], [[nearestKeyX(featIdKey, end) nearestKeyY(featIdKey, end)]], nearestKeyZ(featIdKey,1), pixLenThr, intrMat, pnp_ang_est_max_margin);
depthPrv_temp = depthPrv;
depthCur_temp = depthCur;

img_prev = rgb2gray(im2double(imgPrv));
img_curr = rgb2gray(im2double(imgCur));
% depthPrv_temp(isnan(depthPrv_temp)) = 0;

[xGrid,yGrid] = meshgrid(1:size(depthPrv,2), 1:size(depthPrv,1));
if 0
    [XYZ_prv] = GetXYZFromDepth(intrMat, [xGrid(:) yGrid(:)],depthPrv(:));
    [XYZ_cur] = GetXYZFromDepth(intrMat, [xGrid(:) yGrid(:)],depthCur(:));
else
    [XYZ_prv] = GetXYZFromDepth(intrMat, [nearestKeyX(inIdKey, end-1) nearestKeyY(inIdKey, end-1)], nearestKeyZ(inIdKey, end-1));
    [XYZ_cur] = GetXYZFromDepth(intrMat, [nearestKeyX(inIdKey, end) nearestKeyY(inIdKey, end)], nearestKeyZ(inIdKey, end));
end
XYZ_prv = XYZ_prv(~isnan(XYZ_prv(:,3)),:);
XYZ_cur = XYZ_cur(~isnan(XYZ_cur(:,3)),:);


TCur = [reshape(thetaList(end,1:9),3,3) thetaList(end,10:12)';0 0 0 1];
TPrv = [reshape(thetaList(end-1,1:9),3,3) thetaList(end-1,10:12)';0 0 0 1];
deltaTGT = TCur \ TPrv;

lastMat = [reshape(vslMat(end,1:9),3,3) vslMat(end,10:12)';0 0 0 1];
lastMatPNP = [reshape(vslMatPNP(end,1:9),3,3) vslMatPNP(end,10:12)';0 0 0 1];
newMat = [rodrigues(vslPoseVec(1:3)) vslPoseVec(4:6); 0 0 0 1]*lastMat;
initPose = [rodrigues(vslPoseVec(1:3)) vslPoseVec(4:6);0 0 0 1];

newMat_inv_pnp = lastMatPNP*inv([rodrigues(vslPoseVec(1:3)) vslPoseVec(4:6); 0 0 0 1]);


if 1
    newMat_inv = lastMat*inv([rodrigues(vslPoseVec(1:3)) vslPoseVec(4:6); 0 0 0 1]);
elseif 1
    
% %     newMat_inv_pnp = lastMatPNP*inv([rodrigues(vslPoseVec(1:3)) vslPoseVec(4:6); 0 0 0 1]);
    
    depthPrv_temp(depthPrv_temp > 200*1000) = nan;
    depthCur_temp(depthCur_temp > 200*1000) = nan;
    depthPrv_temp(yMatPrv > yRng(1) | yMatPrv < yRng(2)) = nan;
    depthCur_temp(yMatCur > yRng(1) | yMatCur < yRng(2)) = nan;
    
    depthPrv_temp(depthPrv_temp > zRng(1) | depthPrv_temp < zRng(2)) = nan;
    depthCur_temp(depthCur_temp > zRng(1) | depthCur_temp < zRng(2)) = nan;
    
    [pose_rel, score, warped_image, valid_mask, weightMap,  warped_z, warped_z_mask] = estimateVisualOdometry(img_curr, img_prev, depthCur_temp, depthPrv_temp, intrMat, num_levels,initPose, 0, initial_sigma,default_dof, para);
    pose_rel0 = pose_rel;
    if 0
     [epsilon,kmax,initial_sigma,default_dof,DSR,scalefactor] = GetInitialParas();
%         DSR = num_levels;
        InParas = GetInParas('sROS',DSR);
        InParas = [intrMat(1,1) intrMat(2,2) intrMat(1,3) intrMat(2,3)];
        [G1, e1, r1, W1, weightMap1 ]= EstimateCameraMotion(img_prev,depthPrv_temp,img_curr,depthCur_temp,pose_rel0,InParas,epsilon,kmax,initial_sigma,default_dof,1);
        pose_rel = G1;
        [warped_image, valid_mask, warped_z, warped_z_mask] = warpImage(img_curr, depthPrv_temp, G1, intrMat);
        score = mean((warped_image(valid_mask) - img_prev(valid_mask)).^2);
    end
    
    [warped_image1, valid_mask1, warped_z1, warped_z_mask1] = warpImage(img_curr, depthPrv_temp, [rodrigues(vslPoseVec(1:3)) vslPoseVec(4:6);0 0 0 1], intrMat);
    score1 = mean((warped_image1(valid_mask1) - img_prev(valid_mask1)).^2);
    
    
    if 1
        figure(4),clf;subplot(2,2,1),imshowpair(warped_image, img_prev);title('opt');subplot(2,2,2),imshowpair(warped_image1, img_prev);title('init'); subplot(2,2,3),imshow(abs(warped_image - img_prev).*valid_mask, []);title(num2str(score));subplot(2,2,4),imshow(abs(warped_image1 - img_prev).*valid_mask1);title(num2str(score1));drawnow;
    end
    
    
    dsfjhgdghfdhgdfhgccffdjgfdfgdjchfkfhhvhgfdjhgj = 1;
    
    
    if 0
        [xGridAll, yGridAll] = meshgrid(1:obj.camModel.imgSize(2)*(1/2^obj.scaleLvl), 1:obj.camModel.imgSize(1)*(1/2^obj.scaleLvl));
        PixAll = [xGridAll(:) yGridAll(:)];
        
        rgbMatPrv = [reshape(imgPrv(:,:,1),[],1) reshape(imgPrv(:,:,2),[],1) reshape(imgPrv(:,:,3),[],1)];
        rgbMatCur = [reshape(imgCur(:,:,1),[],1) reshape(imgCur(:,:,2),[],1) reshape(imgCur(:,:,3),[],1)];
        
        [XYZ_prv] = GetXYZFromDepth(intrMat, PixAll,depthPrv_temp(:));
        [XYZ_cur] = GetXYZFromDepth(intrMat, PixAll,depthCur_temp(:));
        
        xyzMat_prv(:,:,1) = reshape(XYZ_prv(:,1), size(imgPrv,1), size(imgPrv,2));xyzMat_prv(:,:,2) = reshape(XYZ_prv(:,2), size(imgPrv,1), size(imgPrv,2));xyzMat_prv(:,:,3) = reshape(XYZ_prv(:,3), size(imgPrv,1), size(imgPrv,2));
        xyzMat_cur(:,:,1) = reshape(XYZ_cur(:,1), size(imgPrv,1), size(imgPrv,2));xyzMat_cur(:,:,2) = reshape(XYZ_cur(:,2), size(imgPrv,1), size(imgPrv,2));xyzMat_cur(:,:,3) = reshape(XYZ_cur(:,3), size(imgPrv,1), size(imgPrv,2));
        if 0
            ...
            ptCloudPrv = pointCloud(xyzMat_prv,'Color',imgPrv);
            ptCloudCur = pointCloud(xyzMat_cur,'Color',imgCur);
        end
        ptCloudPrv_filt = ptcloud_edge_filter(ptCloudPrv);
        ptCloudCur_filt = ptcloud_edge_filter(ptCloudCur);
        
        rkhs_se3 = rkhs_se3_registration();
        rkhs_se3.set_ptclouds(ptCloudPrv_filt, ptCloudCur_filt);
        rkhs_se3.align();
    ...
    if 0
        figure,subplot(2,2,1),imshowpair(warped_image, img_prev);title('opt');subplot(2,2,2),imshowpair(warped_image1, img_prev);title('init'); subplot(2,2,3),imshow(abs(warped_image - img_prev).*valid_mask, []);title(num2str(score));subplot(2,2,4),imshow(abs(warped_image1 - img_prev).*valid_mask1);title(num2str(score1));
        
%         depthErrMat = abs(depthPrv - depthPrvGT123);
        depthErrMat = (depthPrv - depthPrvGT123);
        depthErrMat(isnan(depthErrMat)) = 0;
        depthErrMat(depthErrMat > 2000) = 0;
        figure,subplot(1,2,1),imshow(depthErrMat, []);subplot(1,2,2);imshow(weightMap, []);
        
        
        depthErrCur = abs(depthCur - warped_z);
        depthErrCur(~warped_z_mask) = 0;
        depthErrCur(isnan(depthCur)) = 0;
        depthErrCurMask = depthErrCur > 0;
%         depthErrPrvGT = 
    end
    newMat_inv = lastMat*inv(pose_rel);
...
%         DSR = num_levels;
        InParas = GetInParas('sROS',DSR);
        InParas = [intrMat(1,1) intrMat(2,2) intrMat(1,3) intrMat(2,3)];
        [G1 e1 r1 W1, weightMap1 ]= EstimateCameraMotion(img_prev,depthPrv_temp,img_curr,depthCur_temp,initPose,InParas,epsilon,kmax,initial_sigma,default_dof,1);
        [warped_image2, valid_mask2, warped_z2, warped_z_mask2] = warpImage(img_curr, depthPrv_temp, G1, intrMat);
        score2 = mean((warped_image2(valid_mask2) - img_prev(valid_mask2)).^2);

        figure(5),clf;subplot(2,2,1),imshowpair(warped_image2, img_prev);title('opt');subplot(2,2,2),imshowpair(warped_image1, img_prev);title('init'); subplot(2,2,3),imshow(abs(warped_image2 - img_prev).*valid_mask2, []);title(num2str(score2));subplot(2,2,4),imshow(abs(warped_image1 - img_prev).*valid_mask1);title(num2str(score1));drawnow;
    ...
    if 1
%         [tform,rmes] = pcregrigid(pointCloud(XYZ_prv),pointCloud(XYZ_cur),'InlierRatio', 0.8,'MaxIterations',100,'InitialTransform',affine3d(pose_rel'),'Extrapolate',true);
        [tform,rmes] = pcregrigid(pointCloud(XYZ_prv),pointCloud(XYZ_cur),'InlierRatio', 1,'MaxIterations',100,'Extrapolate',true);
        tf = tform.T';
    else
        SmartRejection = 50; 0.1; 100;
        [R, t, ER, maxD] = icp(XYZ_cur', XYZ_prv', 100, 'Matching','kDtree','SmartRejection',SmartRejection);
        tf = [R t;0 0 0 1];
    end
    if 0
        mapperPrv = (tf(1:3,1:3)*XYZ_prv' + repmat(tf(1:3,4), 1, size(XYZ_prv,1)))';
        figure,pcshowpair(pointCloud(XYZ_cur), pointCloud(mapperPrv))
    end
    newMat_inv = lastMat*inv(tf);
end
% vslMat = [vslMat; reshape(newMat(1:3,1:3), 1, 9) newMat(1:3, 4)'];
vslMat = [vslMat; reshape(newMat_inv(1:3,1:3), 1, 9) newMat_inv(1:3, 4)'];
vslMatPNP = [vslMatPNP; reshape(newMat_inv_pnp(1:3,1:3), 1, 9) newMat_inv_pnp(1:3, 4)'];
...
[LocalTraceList, unValidFeatId, validFeatId, traceLenList, dataTmp, dispErrMat, cur_id_lk_gt2, pixGT_temp_stack2, trackingLostId] = PlotMultiTrace(obj, 8, imgCur, obj.traceManager.X, obj.traceManager.Y, obj.traceManager.Z, obj.traceManager.ZGT, intrMat, princpPtL,princpPtR, baseline, obj.configParam, obj.colorMat, b2cPmat, thetaList, cur_lk_gt, pix_z);
...
if 0
    ...
    if 1
        ...
        frameNum = size(obj.traceManager.X, 2);
        traceManager_bak = obj.traceManager;
        if 1
            if ~isempty(trackingLostId)
                lostId = [1:max(trackingLostId)]';
                lostId0 = lostId;
                lostId = unique([lostId;find(obj.traceManager.X(:,end) == -1)]);
%                 obj.traceManager.X(lostId,:) = [];
%                 obj.traceManager.Y(lostId,:) = [];
%                 obj.traceManager.Z(lostId,:) = [];
%                 obj.traceManager.ZGT(lostId,:) = [];
            else
                lostId = [find(obj.traceManager.X(:,end) == -1)];
            end
                ...
                frameNum2 = size(obj.traceManager.X, 2);
                
                if frameNum2 >= 27 && frameNum ~=1
                    dskgjb = 1;
                end
                
        else
            if isempty(unValidFeatId)
                frameRng = frameNum-size(LocalTraceList{1,1}.ptIcsX,2)+1 : frameNum;
            else
                frameRng = frameNum-size(LocalTraceList{1,1}.ptIcsX,2)+1 : frameNum;
            end
            if frameRng(1) > 1
                %             obj.traceManager.X(:,1:frameRng(1)-1) = -1;
                obj.traceManager.X(:,1:frameRng(1)-0) = -1;
                lostId = find(sum(obj.traceManager.X')' == -1.*size(obj.traceManager.X, 2) | obj.traceManager.X(:,end) == -1);
                obj.traceManager.X(lostId,:) = [];
                obj.traceManager.Y(lostId,:) = [];
                obj.traceManager.Z(lostId,:) = [];
                obj.traceManager.ZGT(lostId,:) = [];
                
                %             obj.traceManager.X(:,1:frameRng(1)-1) = [];
                %             obj.traceManager.Y(:,1:frameRng(1)-1) = [];
                %             obj.traceManager.Z(:,1:frameRng(1)-1) = [];
                %             obj.traceManager.ZGT(:,1:frameRng(1)-1) = [];
                
            end
        end
    else
        obj.traceManager.X(unValidFeatId,[]) = [];
        obj.traceManager.Y(unValidFeatId,[]) = [];
        obj.traceManager.Z(unValidFeatId,[]) = [];
        obj.traceManager.ZGT(unValidFeatId,[]) = [];
    end
end
% if length(thetaList) > 2
...
traceManager_use2 = obj.traceManager;


if 1
    usedId = find(traceManager_use2.X(1,:) > 0);
    traceManager_use = traceManager_use2;
    traceManager_use.X = traceManager_use2.X(:,usedId(1):end);
    traceManager_use.Y = traceManager_use2.Y(:,usedId(1):end);
    traceManager_use.Z = traceManager_use2.Z(:,usedId(1):end);
    traceManager_use.ZGT = traceManager_use2.ZGT(:,usedId(1):end);
    MakeDirIfMissing(probPath); 
    save(fullfile(probPath, sprintf('traceData_%05d.mat', imgInd)), 'traceManager_use','-v7.3');
end
dghdg = 1;
...
if 0
    ...
    if 0 % imgInd >= length(thetaListGT) - 2
...
function [LocalTraceList, unValidFeatId, validFeatId, traceLenList, data, dispErrMat, cur_id_lk_gt2, pixGT_temp_stack2, trackingLostId] = PlotMultiTrace(obj, figNum, img, localX, localY, localZ, localZGT, intrMat, princpPtL,princpPtR, baseline, ConfigParam, colorMat, b2cPmat, accumP2CRef, cur_lk_gt, pix_z)
global newRender ShiftLK newShift ReplaceOnlyP2 ReplaceOnlyP2Z ShiftByCoord doubleShift mean_x_err...
    rou shiftPixThr doubleShift2 err_x UseDoubleShift Rou UseEx Distri thetaListGT pixLenThr deOutlierThr
...
thetaListGTTemp = [2;3]; %thetaListGT(1:size(obj.accumP2CPNP2,1));
...
idLost = find(sum(localX)' == -1.*size(localX,1));
featId = find(localX(:,end) > 0 & localY(:,end) > 0);
% featId = setdiff(featId0, idLost);
...
pnp_ang_est_max_margin = [deg2rad([-1 1]) deOutlierThr];
pnp_ang_est_max_margin_large = [deg2rad([-6 6]) 3.7];
dispMat = [];
dispMatGT = [];
featSize = [];
dispErrMat = [];
...
%         if length(featId2) < 10  || traceNumUniq(i) > numThr %  19 %15  % 17  % 15 %15 % 200  %130   % 14 %80 %80; % 80;  %50 % 80 %50 %130  %15 % 40
        if length(featId2) < 1  || traceNumUniq(i) > numThr %  19 %15  % 17  % 15 %15 % 200  %130   % 14 %80 %80; % 80;  %50 % 80 %50 %130  %15 % 40
            unValidFeatId = [unValidFeatId; featId(featId2)];
            trackingLostId = [trackingLostId; featId(featId2)];
            continue;
        end
        ...
        if 0
            inId = PnPDeoutlier(find(inTrackFlag00),[localTraceX0(inTrackFlag00,1) localTraceY0(inTrackFlag00,1)], [localTraceX0(inTrackFlag00,end) localTraceY0(inTrackFlag00,end)], localTraceZ0(inTrackFlag00,1), pixLenThr, intrMat, pnp_ang_est_max_margin);
        else
            inId = find(inTrackFlag00);
        end
        if 0
            if length(inId) < 30
...
pnp_ang_est_max_margin_2 = [deg2rad([-0.05 0.05]) deOutlierThr];
roundingPrecision = 0.00000001;
...
if ReplaceOnlyP2
    ...
    p2cBodyRotAng_2_onlyP2 = 1; %p2cBodyRotAng_2;
end
...
if draw
    ...
    if 0
        plot(localX(featId, end), localY(featId, end), '.r');
        title(sprintf('%d tracked feature points',length(featId)))
    else
        plot(localX(validFeatId, end), localY(validFeatId, end), '.r');
        title(sprintf('%d tracked feature points\nframe Id: %d',length(validFeatId), size(localX,2)))
    end
...
global deOutlierThr
pnp_ang_est_max_margin_2 = [deg2rad([-0.005 0.005]) deOutlierThr];
step = 0.00001/10;
