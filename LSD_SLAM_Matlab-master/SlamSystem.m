classdef SlamSystem < handle
    properties
        ...
        mappingTrackingReference % for mappinng
        
        newKFTrackingReference
        candidateTrackingReference
        map;
        K;
        width, height;
        currentKeyFrame;
    ...
    methods 
       function obj = SlamSystem(K, w , h)
        ...
        function trackFrame(obj,trackingNewFrame)
            ...
            %}
            if(size(obj.trackingReference.keyFrame,1) == 0 | obj.trackingReference.keyFrame ~= obj.currentKeyFrame | obj.currentKeyFrame.depthHasBeenUpdatedFlag)
                obj.trackingReference.importFrame(obj.currentKeyFrame);
                obj.currentKeyFrame.depthHasBeenUpdatedFlag = false;
            end
            thisToOther = obj.rgbdODAMTrack( trackingNewFrame); % k2c update: p2c accumulate //// Old2New
            
            
            if 0
                a = obj.currentKeyFrame.warpImage(thisToOther);  % inv(T):New2Old,  T:Old2New
                figure,imshow(trackingNewFrame.imgRGB)
                figure,imshow(a.imgRGB)
            end
            
            
            trackingNewFrame.pose.thisToParent_raw = inv(thisToOther) ;% New2Old = inv(Old2New);  c2k sim3FromSE3(toSophus(referenceToFrame.inverse()),1); 
            trackingNewFrame.pose.trackingParent =  obj.trackingReference.keyFrame.pose;%obj.currentKeyFrame.pose;%trackingReference.keyframe.pose;
            ...
            trackingNewFrame.prepareForStereoWith( obj.currentKeyFrame , thisToOther, obj.K, 1); % k2c
            if 0
                [y , x] = meshgrid(3:obj.height-3,3 :obj.width - 3);
            else
                [x , y] = meshgrid(3 :obj.width - 3,3:obj.height-3);
            end
            %[flag,pepx,pepy, x, y] = obj.map.makeAndCheckEPLMat( x , y ,trackingNewFrame);
            obj.map.observeDepthUpdate( x , y ,trackingNewFrame); % otherTothis, this is key
            %trackingNewFrame.P3_to_P2 * T
            
            %change it to candidate key feame
            % and update "obj.otherDepthMap"
             obj.map.createKeyFrame(trackingNewFrame);
        ...
        function T = rgbdODAMTrack(obj,  trackingNewFrame)
               ...
               figure(2), subplot(2,3,1), imshow(imgB_I,[0 255]);
                ...
                for downsample_level=3:3 %:-1:0 % 2:2 % 3:-1:3
                    %a_k = [0 0 0 0 0 0];
                ...
                T = rgbd_odom.parametersToTransform(a_k);
                if (verbose==true)
                imgB_Z = rgbdImg2.getDepthImage(); % key
                imgB_I = rgbdImg2.getGrayImage(); % cur
                warpA2 = rgbdImg1.warpImage(inv(T));  % T:k2c,  inv(T):c2k
                warpA2_Z = warpA2.getDepthImage();
                warpA2_I = warpA2.getGrayImage();
