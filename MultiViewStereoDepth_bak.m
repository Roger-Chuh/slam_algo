function MultiViewStereoDepth_bak(imgPrv, imgCur, K, C2P)


if 0
    imgPrv = imresize(imread('test00.jpg'), 0.25);
    imgCur = imresize(imread('test09.jpg'),0.25);
    intrMat = [1221.2270770/4	0.0000000	479.5000000/4
        0.0000000	1221.2270770/4	269.5000000/4
        0.0000000	0.0000000	1.000000];
    P2C = [0.9998813487	0.0148994942	0.0039106989
        -0.0148907594	0.9998865876	-0.0022532664
        -0.0039438279	0.0021947658	0.9999898146
        -9.9909793759	0.2451742154	0.1650832670	];
    P2C = [P2C(1:3,1:3) P2C(end,:)'];
    P2C = [P2C;0 0 0 1];
    C2P = P2C;
end
...
img_1 = double(imgPrv);
img_2 = double(imgCur);
% img_1=double(imread('.\image\test00.jpg'));
% img_2=double(imread('.\image\test09.jpg'));
[H, W, ~] = size(img_1);
N=H*W;
img_1_prime=reshape(img_1,1,N,3);

% cameras=fopen('.\image\cameras_PART3.txt','r');
% camera_mat = fscanf(cameras, '%f %f %f', [3,Inf]);
% fclose(cameras);
K_1 =K;
R_1 = eye(3); %camera_mat(:,4:6)';
T_1 = [0;0;0];  %camera_mat(:,7);

K_2 = K;
R_2 = C2P(1:3,1:3); %)camera_mat(:,11:13)';
T_2 = C2P(1:3,4); %camera_mat(:,14);
