function [angOpt, inlierId, angRng, PixUse, pt2dCurUse, DispRefineUse_, angOpt3, inFlag,thetaRngOut,thetaProbOut,angOptP2C, angOptK2C_, p2cOnlyP2_111,...
    ...
    REFAngList4,...
    accumP2CTemp,...
    twoOnlyP2List,...readYUV( 'D:\Temp\20200319\dump', 1280, 720, 1);
    thetaPlatform,...
    ...
    meanErrAndStd,...
    p2cErrList,...        
    objPervPtCcsZGolden1,cfg] = RefineZ(cfg, pt2d_check, traceBatch, k2cRef_00, obj, k2cRef,k2cPnp, inlierId0, inlierId2, theta, DepthProbility,  DepthId,  AngleModalOrg,  AngleProbility,prevFeatPtList0,ptCcsZ0,DispRefineUse__,NewDispRng,newDispStep,ProbZTmpReSamp,k2cBody,...
    keyFrameLen ,...
    disparity_resample_prob_threshold,...
    ThetaNoise ,...
    KeyFrameFlagList,...
...
global SHOWPOLYGON DRAWPOLYGON ANGLEONLY USEGOLDENDISP UPDATEDEPTH DEPTHITERNUM probPath ...
    ...
    ProbZCutOffThr FigBase featInd ReplaceNoKey


LocalTrace0 = LocalTrace;


p2cAllFeat = false;  true; false; true; false; true;  false; true; false; true; false; true;

%% 20200327
keepSecondZUpdateFixed = false; true;


thetaOfstList = cfg.thetaOfst;
thetaOfst = cfg.thetaOfst(end);
if length(thetaOfstList) > 1
   if max(abs(thetaOfstList(1:end - 1))) > 0
       thetaOfst = 0;
   end
end
...
replaceKeyDisp0 = true;

replaceKeyDisp2 = false; true;

if isfield(cfg, 'a')
    replaceKeyDisp0 = false;
    replaceKeyDisp2 = true;
end
...
curUpdateTransDispRatio = 0.2; 0; 0.2;  1; 0.2; 1; 0.2; 1; 0.2;

if keyFrameLen > 2 && keyFrameLen <= 1200000
    replaceKeyDisp = true; false; true; false; true;
else
    replaceKeyDisp = false;
end

% replaceKeyDisp = true;


if replaceKeyDisp0 && keyFrameLen > 2
    if replaceKeyDisp
        
        [pix_CinK_, transXYZ_CinK_] = VisualLocalizer.GetGtTrace2(b2cPmat, -ANGOpt(end), [LocalTrace.ptIcsX(:,end-1) LocalTrace.ptIcsY(:,end-1)],LocalTrace.ptCcsZ(:,end-1),intrMat);
        %     [pix_CinK_KinC_, transXYZ_CinK_KinC_] = VisualLocalizer.GetGtTrace2(b2cPmat, angOptList, [LocalTrace.ptIcsX(:,1) LocalTrace.ptIcsY(:,1)],transXYZ_CinK(:,3),intrMat);
        transDisp_CinK_ = (intrMat(1,1).*norm(obj.camModel.transVec1To2))./(transXYZ_CinK_(:,3) + (princpPtR(1) - princpPtL(1)));
        ...
        if 0
            ...
            LocalTrace.ptCcsZ(:,1) = transXYZ_CinK_(:,3);
            LocalTrace.dispList(:,1) = transDisp_CinK_;
            LocalTrace.DispRng = transDisp_CinK_ + disparityRng;
            saghdafg = 1;
            
            ZZVec1___ = [];ProbZ___ = [];
            for jk11 = 1 : length(LocalTrace.featId)
                [ProbZ___(jk11,:), ZZVec1___(jk11,:)] = probDensity(transDisp_CinK_((jk11),:), ConfigParam.disparity_sigma,ConfigParam.disparity_beta,ConfigParam.reproj_beta, LocalTrace.DispRng(jk11,:),ConfigParam.disparity_sample_interval, 'disparity');
            end
            
            LocalTrace.sampleZ = ZZVec1___;
...
% % %                         keyDispWeighted = (1 - curUpdateTransDispRatio).*keyDispWeighted + curUpdateTransDispRatio.*LocalTrace.transDisp{iop,3};
% % %                     end
                    keyDispWeighted_all = [keyDispWeighted_all;  LocalTrace.transDisp{iop,3}];
                end
                if (size(LocalTrace.transDisp,1)) == 1
...
% %                     keyDispWeighted = (keyDispWeighted_all(end,:));
            ...
            keyDispWeighted_bak = keyDispWeighted;
            if ~replaceKeyDisp2
                keyDispWeighted = LocalTrace0.dispList(:,1)';
                
            end
            
            
            
            keyDepthWeighted = (intrMat(1,1).*norm(CamModel.transVec1To2))./(keyDispWeighted + (princpPtR(1) - princpPtL(1)));
            keyDepthWeighted_bak = (intrMat(1,1).*norm(CamModel.transVec1To2))./(keyDispWeighted_bak + (princpPtR(1) - princpPtL(1)));
            
            LocalTrace.ptCcsZ(:,1) = keyDepthWeighted';
            
            LocalTrace.dispList(:,1) = keyDispWeighted';
            LocalTrace.DispRng = keyDispWeighted' + disparityRng;
            
            ZZVec1___ = [];ProbZ___ = [];
            for jk11 = 1 : length(LocalTrace.featId)
                [ProbZ___(jk11,:), ZZVec1___(jk11,:)] = probDensity(keyDispWeighted(1, (jk11)), ConfigParam.disparity_sigma,ConfigParam.disparity_beta,ConfigParam.reproj_beta, LocalTrace.DispRng(jk11,:),ConfigParam.disparity_sample_interval, 'disparity');
            end
        ...
        if 0
            %         figure,subplot(1,2,1); plot(LocalTrace.dispList(:,1) -LocalTrace0.dispList(:,1) );title('disp');subplot(1,2,2),plot(LocalTrace.ptCcsZ(:,1) -LocalTrace0.ptCcsZ(:,1) );title('depth');
            figure,subplot(1,2,1); hist(LocalTrace.dispList(:,1) -LocalTrace0.dispListGT(:,1) );title('after change');subplot(1,2,2),hist(LocalTrace0.dispList(:,1) -LocalTrace0.dispListGT(:,1) );title('before change');
        end
    else
...
if 1
    k2cRef_pnp = obj.accumP2CPNP2(end) - obj.accumP2CPNP2(length(obj.accumP2CPNP2) - keyFrameLen + 0 + 1);
% else
%     p2cRef_pnp = obj.accumP2CPNP2(end) - obj.accumP2CPNP2(end - 1);
end
p2cRef_pnp = obj.accumP2CPNP2(end) - obj.accumP2CPNP2(end - 1);
 
%% 20200402 PorbZInit change disparity_sigma from 15 to 0.2;
if 1  %    ~isfield(cfg, 'a')
    LocalTrace.probZ = ones(size(LocalTrace.probZ));
    
    ZZVec1___123 = [];ProbZ___123 = [];
    for jk11 = 1 : length(LocalTrace.featId)
        [ProbZ___123(jk11,:), ZZVec1___123(jk11,:)] = probDensity(LocalTrace.dispList((jk11), 1), obj.configParam.disparity_sigma,obj.configParam.disparity_beta,obj.configParam.reproj_beta, LocalTrace.DispRng(jk11,:),obj.configParam.disparity_sample_interval, 'disparity');
    end
    
    ProbZ___123 = ProbZ___123./repmat(max(ProbZ___123')',1,size(ProbZ___123,2));  
    LocalTrace.probZ = ProbZ___123;
...
if keyFrameLen <= FrmNumWithGloden + 1
    ...
    if 1
        ...
        if 0 % old param config before 20200202
            Rati1 = 0; 0.8; 0; 0.8; 0; 0.5; 0.8; 0; 0.5; 0.8; 0.8; 0.8; 0; 0.4; 0.3; 0.5; 0; 0.8; 0.0001; 0.95;
            Rati2 = 0; 0.8;
        else
            if 0
                Rati1 = 0.3; 0.5; 0.3;  0.2;  0; 0.8; 0; 0.8; 0; 0.5; 0.8; 0; 0.5; 0.8; 0.8; 0.8; 0; 0.4; 0.3; 0.5; 0; 0.8; 0.0001; 0.95;
                Rati2 = 0.3; 0.5; 0.3;  0.2;  0; 0.8;
            else
                %% 20200327
                Rati1 = 0; 0.3; 0.5; 0.3;  0.2;  0; 0.8; 0; 0.8; 0; 0.5; 0.8; 0; 0.5; 0.8; 0.8; 0.8; 0; 0.4; 0.3; 0.5; 0; 0.8; 0.0001; 0.95;
                Rati2 = 0; 0.3; 0.5; 0.3;  0.2;  0; 0.8;
            end
        end
...
if obj.switchDepth 
    depthMapCur_1 = obj.depthGT;
else
    depthMapCur_1 = obj.depthVisual;
end
depthMapCurGT_1 = obj.depthGT;



dispMapCur_1 = (intrMat(1,1).*norm(obj.camModel.transVec1To2))./(depthMapCur_1 + (princpPtR(1) - princpPtL(1)));
dispMapCur_1(dispMapCur_1 < 0) = nan;

dispMapCurGT_1 = (intrMat(1,1).*norm(obj.camModel.transVec1To2))./(depthMapCurGT_1 + (princpPtR(1) - princpPtL(1)));
% % % % % try
...
%             end
    ...
    dispCurList_gt = LocalTrace.dispListGT(:,end);
...
if keyFrameLen > 2
    ...
    if obj.switchDepth
        ...
        dispPrvList_gt = LocalTrace.dispListGT(:,end-1);
...
ProbZTmp_update_norm_sum_tmp_p2c = zeros(size(LocalTrace.probZ));
    ...
    if ~isempty(inlierId)
        ...
        if 1
            if keyFrameLen == 2 %isempty(LocalTrace.probZ)
                ...
                dispGTTmp(isnan(disparityError)) = dispList(isnan(disparityError));
                disparityError = dispList(:) - dispGTTmp(:);
%                 disparityError(isnan(disparityError)) = 0;   
                ...
                try
                    [DispRng, ProbZTmpReSamp2, DispKeyInPrvRng, dispPInK_intersect2view, dispPInK] = IntersectCone(obj, inlierId, inlierIdPrv, imgKeyL, imgPrvL,b2c, DispRng,dispPrvList_, DepthProbility,DepthId,ConfigParam,KeyFrameFlagList,thetaRngMat,thetaProbMat,LocalTrace);
                catch
                    svkbj = 1;
                end
                if NewPrvDispResamp
                ...
                if 0
                    ...
                    zList(:) = LocalTrace.ptCcsZ(:,1);
                    if 0  %% 20200214   might fix bug: sth wrong in accumP2C  % shit never mind
...
%                         k2cBodyPmat = PointCoordTransformer(BaseLocalizer.RotMatFromAngle(k2cRef0),zeros(3,1));
                    ...
                    if ~isempty(pt2d_check)
                        [~, err_check] = NormalizeVector(pt2d_check(:,4:5) - pt2d_check(:,6:7));
                        
                        commonIdCheck = find(ismember(pt2d_check(:,1:2), Pix, 'rows'));
                        commonIdLocal = find(ismember( Pix,pt2d_check(:,1:2), 'rows'));
...
%                         depthList__ = depthList__(depthList__ > 0);
                ...
                if keyFrameLen > 2
                    ...
                    DispRngStepP2C = mean(diff(DispRngP2C'))';
                    [~, depthGTIndAll_1] = RoundingDispErr2(DispRngP2C(:,(size(DispRngP2C,2)+1)/2),dispPrvList_gt, DispRngStepP2C,DispRngP2C);
                    
                    depthGTIndAll_1(isnan(depthGTIndAll_1)) = 1;
                    
                    ProbZTmp_update_norm_sum_tmp_p2c = zeros(size(DispRngP2C));
%                     ProbZTmp_update_norm_sum_tmp(depthGTInd11) = ProbZTmp_update_norm_sum(depthGTInd11);
                    ProbZTmp_update_norm_sum_tmp_p2c(depthGTIndAll_1) = 1;
                    ...
                    if 1
                        [projDispMatAll, thetaProbP2C,idMP2C,angOptP2C, ~, ~, ~, ~,~,~,~,~,ProbReprojVecAllP2C, ProbReprojVecRAllP2C, ~, ~, ~,~,~, ~, ~,~,~,~,~,~,~,thetaExp_tmpP2C] = ReplayTheta(obj, thetaRngP2C, b2c, prvCcsXYZAll, intrMat, pt2dPrv, disparityRng, baseline, ZZVecPrv1, L2R, pt2dCur, pt2dCurR, ProbZP2C, ProbZP2C_norm, imgCur, imgCurR,thetaSampP2C,r_cam,tx, ty, tz);
                        [angOptP2C_pnp, ~, ~] = VisualLocalizer.RotationAngleEstimate_PNP_(pnp_ang_est_max_margin,intrMat,pt2dPrv,pt2dCur,ZTruePrv, [1:size(ZTruePrv,1)]',true(size(ZTruePrv,1),1),b2c,k2cRef_00 - REFAngList(end));
                        
                        % % % %                                     angOptP2C = angOptP2C - deg2rad(0.01);
                        
                        angOptP2C__1 = angOptP2C;
                        obj.p2cErrComp = [obj.p2cErrComp; [traceBatch keyFrameLen-1 rad2deg(angOptP2C__1 - angOptP2C_pnp) rad2deg(angOptP2C__1 - p2cRef_pnp)]];
                ...
                if 0
                    ProbReprojVecAllInit = ProbReprojVecAll;
                end
                ...
                if keyFrameLen == 2
                    ...
                    if 0
                        ...
                        accumP2CTemp = [accumP2CTemp; accumP2CTemp(end) + angOptP2C__1];
                    end
                end
                ...
                ProbZNormSum11 = ProbZ./(repmat(sum(ProbZ')', 1, size(ProbZ, 2)));
                DispExp = sum(DispRng'.*ProbZNormSum11')';
                thetaPnP = PureRotationAngPredictor.pnpLite_1([],intrMat,[],obj.coordSysAligner,k2cRef,[],[],[],Pix ,pt2dCur, DispExp, CamModel, obj.scaleLvl );
...
%                 DispRefineUse_
                ...
                for ft = 1 : IterThetaNum-1
                    ...
                    if 1 % 20191123 use ExpCoord_OnlyP2 to update angOpt
                        [projDispMatAll, thetaProb,idM,angOpt, ReprojErrVecTmp, ReprojErrVecRTmp, ReprojErrVecTmpMatT, ReprojErrVecRTmpMatT,ValidFeatFlag,ValidFeatFlag0,ValidFeatFlagFusion,ValidFeatFlagQuant,ProbReprojVecAll, ProbReprojVecRAll, PixKeyVecTmp, PixKeyVecRTmp, Probb, ...
                ...
                if 0
                    ...
                    if 1     
                        try
                            ...
                            thetaProbK2C_ = []; angOptK2C_ = []; thetaExp_tmpK2C = [];
                        end
                    else
...
% %                             clf;
                        ...
                        if 1  % traceBatch == 67
                            figure(FigBase + 20),subplot(figRow, figCol,1);
                            cla;
                            hold on;plot(rad2deg(thetaSamp+ (k2cRef - k2cRef_00)), thetaProb);grid on; title('1st theta from only p2');
                            if 1
                                plot(rad2deg(angOpt3 - k2cRef_00), max(thetaProb),'or');
                            end
                        end
                    end
                            ...
                            if  1 % ONLYP2
                                ...
                                if 0 %~ONLYP2_2
                                    ...
                                    if 0
                                        ...
                                        if 0
                                            obj.configParam.reproj_sigma = ConfigParam.reproj_sigma_update_z ;
                                            obj.configParam.reproj_sigma_right = ConfigParam.reproj_sigma_update_z_right;
                                        end
                                    end
                                    
                                    
                                    
                                    [projDispMatAll, thetaProb__,idM__,angOpt__, ReprojErrVecTmp, ReprojErrVecRTmp, ReprojErrVecTmpMatT, ReprojErrVecRTmpMatT,ValidFeatFlag,ValidFeatFlag0,ValidFeatFlagFusion,ValidFeatFlagQuant,ProbReprojVecAll, ProbReprojVecRAll, PixKeyVecTmp, PixKeyVecRTmp, Probb, ...
                                        validFeatFlagFusion,validFeatFlag, ProbReprojVec, ProbReprojVecR,bb,b,pixKeyVecTmpXCoordMatStack,pixKeyVecTmpYCoordMatStack,pixKeyVecRTmpXCoordMatStack,pixKeyVecRTmpYCoordMatStack,~, errEpiline, angOptEpi,depthC2KInd_ind, bbbOpt2]...
                                        = ReplayTheta(obj, thetaRng, b2c, keyCcsXYZAll, intrMat, Pix, disparityRng, baseline, ZZVec1, L2R, pt2dCur, pt2dCurR,ProbZ, ProbZTmp_norm, imgCur, imgCurR,thetaSamp,r_cam,tx, ty, tz);
                                    ...
                                    ConfigParam.reproj_sigma_right = ConfigParam.reproj_sigma_right0;
                                    if 0
                                        obj.configParam.reproj_sigma = ConfigParam.reproj_sigma0 ;
                                        obj.configParam.reproj_sigma_right = ConfigParam.reproj_sigma_right0;
                                    end
                                    
                                    ProbReprojVecAll00 = ProbReprojVecAll;
                                    ProbReprojVecAll0 = ProbReprojVecAll;
                                    projDispMatAll00 = projDispMatAll;
                                    projDispMatAll0 = projDispMatAll;
                                    
                                    dlt_noise = deg2rad(thetaOfst);
                                    
                                    if dlt_noise ~= 0
                                        dlt_noise;  %  = deg2rad(0.1*(rand(1,1) - 0.5));
%                                         dlt_noise = thetaOfst;
                                        [projDispMatAllInit,~,~,~, ~, ~, ~, ~,~,~,~,~,ProbReprojVecAllInit, ~, ~, ~, ~, ...
                                            ~,~, ~, ~,~,~,~,~,~,~,~, ~, ~,~,bbbOpt2]...
                                            = ReplayTheta(obj, thetaRng +dlt_noise , b2c, keyCcsXYZAll, intrMat, Pix, disparityRng, baseline, ZZVec1, L2R, pt2dCur, pt2dCurR,ProbZ, ProbZTmp_norm, imgCur, imgCurR,thetaSamp + dlt_noise,r_cam,tx, ty, tz);
                                        thetaRng_1 = thetaRng +dlt_noise;
                                    else
                                        ProbReprojVecAllInit = ProbReprojVecAll;
                                        projDispMatAllInit = projDispMatAll;
                                        thetaRng_1 = thetaRng;
                                    end
                                    
                                    gtThetaId = interp1(thetaRng_1, [1:length(thetaRng_1)],k2cRef_00);
                            ...
                            [projDispMatAll, thetaProb,idM,angOpt, ReprojErrVecTmp, ReprojErrVecRTmp, ReprojErrVecTmpMatT, ReprojErrVecRTmpMatT,ValidFeatFlag,ValidFeatFlag0,ValidFeatFlagFusion,ValidFeatFlagQuant,ProbReprojVecAll, ProbReprojVecRAll, ~, ~, ~, ...
                                ~,~, ~, ~,~,~]...
                                = ReplayTheta(obj, thetaRng, b2c, keyCcsXYZAll, intrMat, Pix, disparityRng, baseline, ZZVec1, L2R, pt2dCur, pt2dCurR,ProbZ, ProbZTmp_norm, imgCur, imgCurR,thetaSamp,r_cam,tx, ty, tz);
                        end
                    ...
                    if 0
                        ...
                        tempTable0 = zeros((1./ConfigParam.modulate_curve_quant_size)*size((imgCur(:,:,1))));
                    end
                    errList = zeros(length(inlierId),2);
                    imgSize = size((imgCur(:,:,1)));
                    if 0
...
%                     k2cBodyPmat = PointCoordTransformer(BaseLocalizer.RotMatFromAngle(k2cRef0),zeros(3,1));
                ...
                if 0
                    ...
                    updatedProbZSum00_0_0 = updatedProbZSum;
                    
                    angSampRng = deg2rad(0.3);  % 0.3; 0.2; 0.1;  0.5
                    angSampNum = round(angSampRng./obj.configParam.theta_sample_step2);
                    angSampNumMid = (size(ProbReprojVecAllInit,1)+1)/2;
                    angSampInd = (angSampNumMid - angSampNum) : (angSampNumMid + angSampNum);
                    
                    angSampInd_gt = interp1(angSampInd, [1:length(angSampInd)], gtThetaId);
                    if ~isfield(cfg, 'angSampInd_gt')
                        cfg.angSampInd_gt = [];
%                         cfg.angSampInd_gt = [];
                    end
                    
                    cfg.angSampInd_gt = [cfg.angSampInd_gt; angSampInd_gt];
                    
                    if 0
                        
                        [pix_c2k, transXYZ_c2k] = VisualLocalizer.GetGtTrace2(b2cPmat, -k2cRef_00, [LocalTrace.ptIcsX(:,end) LocalTrace.ptIcsY(:,end)],LocalTrace.ptCcsZ(:,end),intrMat);
                        
                        
                        [xyz_k] = GetXYZFromDepth(intrMat, pix_c2k,LocalTrace.ptCcsZ(:,1));
                        [pix_k2c, transXYZ_k2c] = VisualLocalizer.GetGtTrace2(b2cPmat, k2cRef_00, [LocalTrace.ptIcsX(:,1) LocalTrace.ptIcsY(:,1)],LocalTrace.ptCcsZ(:,1),intrMat);
                        
                        [pix_k2c_c2k, transXYZ_k2c_c2k] = VisualLocalizer.GetGtTrace2(b2cPmat, -k2cRef_00, [LocalTrace.ptIcsX(:,end) LocalTrace.ptIcsY(:,end)],transXYZ_k2c(:,3),intrMat);
                        %                     [xyz_c2k] = GetXYZFromDepth(intrMat, pix_c2k,transXYZ_c2k);
                    ...
                    if 1
                        LocalTrace.probP2 = [LocalTrace.probP2; [{ProbReprojVecAllInit(angSampInd,:,:)} {projDispMatAllInit(angSampInd,:,:)}]];
                    else
                        LocalTrace.probP2 = [LocalTrace.probP2; [{ProbReprojVecAll(angSampInd,:,:)} {projDispMatAll(angSampInd,:,:)}]];
                    end
                    %                     assert(sum(isnan(ProbReprojVecAllInit(:))) == 0);
                    ...
                    if 0
                        curUpdateRatio = 0.5;
                        curUpdateRatioDisp = 0.9; % 0
                    elseif 1
                        curUpdateRatio = 1; 0; 1; 0.5;
                        curUpdateRatioDisp = 1; 0; 1; 0.9;
                    else
                        curUpdateRatio = 0; 0; 1; 0.5;
                        curUpdateRatioDisp = 0; 0; 1; 0.9;
                    end
                    if 0 % thetaOfst ~= 0
                        curUpdateRatio = 1;
                    end
                    if isempty(cfg.compensateFrmId)
                        startId = 1;
                    else
                        startId = cfg.compensateFrmId(1);
                    end
                    
                    
                    probP2Mat = []; probP2Mat2 = []; probP2Mat3 = [];
                    probDispMat = []; probDispMat2 = []; probDispMat3 = [];
                    for vn = startId : size(LocalTrace.probP2,1)
                        probP2Mat = [probP2Mat; permute(LocalTrace.probP2{vn, 1},[2 1 3])];
                        probP2Mat2 = [probP2Mat2; LocalTrace.probP2{vn, 1}];
                        
                        probDispMat = [probDispMat; permute(LocalTrace.probP2{vn, 2},[2 1 3])];
                        probDispMat2 = [probDispMat2; LocalTrace.probP2{vn, 2}];
                        
                        if vn == startId
                            probP2Mat3 = LocalTrace.probP2{vn, 1};
                            probDispMat3 = LocalTrace.probP2{vn, 2};
                        else
                            probP2Mat3 = (1 - curUpdateRatio).*probP2Mat3 + curUpdateRatio.*LocalTrace.probP2{vn, 1};
                            probDispMat3 = (1 - curUpdateRatioDisp).*probDispMat3 + curUpdateRatioDisp.*LocalTrace.probP2{vn, 2};
                        end
                        
                    end
                    probP2Mat = permute(probP2Mat, [2 3 1]);
                    probDispMat = permute(probDispMat, [2 3 1]);
                    ...
                    if 0
                        ...
                        probDispMat22 = permute(probDispMat2, [1 3 2]);
                        probDispMat22Sum = sum(probDispMat22);
                        probDispMat22Sum = permute(probDispMat22Sum, [3 2 1]);
                        
                    else
                        probP2Mat22 = permute(probP2Mat3, [1 3 2]);
                        probP2Mat22Sum = sum(probP2Mat22);
                        probP2Mat22Sum = permute(probP2Mat22Sum, [3 2 1]);
                        %% 20200401 debug gtDisp - gtTheta change from '0' to '1', scratch that, change back to '0'
                        if 0
                            probDispMat22 = permute(probDispMat2, [1 3 2]);
                        else
                            probDispMat22 = permute(probDispMat3, [1 3 2]);
                        end
                        probDispMat22Sum = sum(probDispMat22);
                        probDispMat22Sum = permute(probDispMat22Sum, [3 2 1]);
                    ...
                    try
                        if 1
                            probP2Mat22_exp = permute(probP2Mat22, [1 3 2]);
                            probP2Mat22_expand = reshape(probP2Mat22_exp, size(probP2Mat22_exp,1) * size(probP2Mat22_exp,2), size(probP2Mat22_exp,3));
                            
                            
                             x_samp = 1 : size(probP2Mat22_exp,1) : size(probP2Mat22_expand,1);
                             x_samp_2 = size(probP2Mat22_exp,1) : size(probP2Mat22_exp,1) : size(probP2Mat22_expand,1);
                        
                        else
                            
                            probP2Mat22_exp = (probP2Mat22);
                            probP2Mat22_expand = reshape(probP2Mat22_exp, size(probP2Mat22_exp,1) , size(probP2Mat22_exp,2)* size(probP2Mat22_exp,3));
                             x_samp = 1 : size(probP2Mat22_exp,2) : size(probP2Mat22_expand,2);
                        x_samp_2 = size(probP2Mat22_exp,2) : size(probP2Mat22_exp,2) : size(probP2Mat22_expand,2);
                        
                        end
                        
                        probP2Mat22_expand = probP2Mat22_expand./max(probP2Mat22_expand);
                        
%                         probP2Mat22_expand = 1-probP2Mat22_expand;
                        probP2Mat22_expand0 = probP2Mat22_expand;
                        
                        
                        probP2Mat22_expand = (255.*probP2Mat22_expand);
                        probP2Mat22_expand(probP2Mat22_expand < (255) & probP2Mat22_expand > (10)) = 255;
                       probP2Mat22_expand = uint8(probP2Mat22_expand);
                       
                       filtLen = 3;
                       line00 = lmatch_detect_lines_use(probP2Mat22_expand, 3, filtLen);
                       
                       if size(line00,1) < length(x_samp)
                           line00 = lmatch_detect_lines_use(probP2Mat22_expand, 3, filtLen - 1.5);
                       end
                       
                        if 0
                            figure(3);hold on;plot(line00(:,1),line00(:,2),'*b');
                        end
                        
                        try
                            metricCcs1 = intrMat\HomoCoord(Pix',1);
                            PtIn2_norm = intrMat\HomoCoord(pt2dCur',1);
                            [angle_resultKtoC,depthCInK] = VisualLocalizer.AnglePredict_Prob(metricCcs1,PtIn2_norm,r_cam,tx, ty, tz);
                            onlyP2_err = rad2deg(angOpt) - angle_resultKtoC;
                        catch
                            asdgkb = 1;
                        end
                        ...
                        KKKK = []; KKKK_unValid = []; k_thr = 0;
                        for ijn = 1 : length(x_samp)
                           id_ijn = find(line00(:,2) >= (x_samp(ijn) - k_thr) & line00(:,2) <= (x_samp_2(ijn) + k_thr)) ;
%                            if ~isempty(id_ijn)
                           if length(id_ijn) == 2
                               lineCoord = line00(id_ijn, :);
                               
                               kkkk = mean(abs((lineCoord(:,4) - lineCoord(:,2))./(lineCoord(:,3) - lineCoord(:,1))));
                               ...
                               KKKK_unValid = [KKKK_unValid; ijn];
                           end
                        end
                        
                        if length(KKKK) <  length(x_samp)
                            KKKK = [KKKK;zeros(length(x_samp)-length(KKKK),1)];
                        end
                        KKKK(KKKK == 0) = nan;
                    catch
                        sagdkhj = 1;
                    ...
                    KKKK_unValid = unique([KKKK_unValid ; find(isnan(bbbOpt2'))]);
                    if ~isfield(cfg, 'KKKK_unValid')
                        cfg.KKKK_unValid = {};
                        cfg.KKKK = {zeros(1,length(KKKK))};
                    end
                    cfg.KKKK_unValid = [cfg.KKKK_unValid; {KKKK_unValid}];
                    try
                        cfg.KKKK = [cfg.KKKK; {KKKK'}];
                    catch
                        cfg.KKKK = [cfg.KKKK; {[]}];
                    end
                    probDispMat22Sum_1 = probDispMat22Sum./repmat(sum(probDispMat22Sum')',1,size(probDispMat22Sum,2));
                    
                    
%                     DispRefine_ = sum(DispRngUse'.*probDispMat22Sum_1')';
                    
                    
                    if 0
                        assert(sum(find(probP2Mat22Sum(:) == 0)) == 0);
                    elseif 0
                    ...
                    if 0
                        if 0
                            updatedProbZSum = probP2Mat22Sum .* updatedProbZSum00_0_0;
                        else
                            updatedProbZSum00_0_1 = updatedProbZSum00_0_0;
                            %                         updatedProbZSum00_0_1(updatedProbZSum00_0_1 > 0.2) = 1;
                            updatedProbZSum00_0_1(updatedProbZSum00_0_1 > 0.1) = 1;
                            updatedProbZSum = probP2Mat22Sum + updatedProbZSum00_0_1;
                        end
                    elseif 1
                ...
                if ~keepSecondZUpdateFixed
                    updatedProbZSum0 = updatedProbZSum;
                else
                    updatedProbZSum0 = ones(size(updatedProbZSum));
                end
                ...
                if 1  % ONLYP2_2
                    if 0
                        ...
                        if 0
                            %% 20200416 unnecessary calculations
                            ...
                            if ~NewFlow
                                ...
                                for ft = 1 : IterThetaNum
                                    ...
                                    if 1 % 20191123 use ExpCoord_OnlyP2 to update angOpt
                                        [projDispMatAll, thetaProb,idM,angOpt, ReprojErrVecTmp, ReprojErrVecRTmp, ReprojErrVecTmpMatT, ReprojErrVecRTmpMatT,ValidFeatFlag,ValidFeatFlag0,ValidFeatFlagFusion,ValidFeatFlagQuant,ProbReprojVecAll, ProbReprojVecRAll, PixKeyVecTmp, PixKeyVecRTmp, Probb, ...
                                            validFeatFlagFusion,validFeatFlag, ProbReprojVec, ProbReprojVecR,bb,b,pixKeyVecTmpXCoordMatStack,pixKeyVecTmpYCoordMatStack,pixKeyVecRTmpXCoordMatStack,pixKeyVecRTmpYCoordMatStack, thetaExp_tmp,errEpiline, angOptEpi,depthC2KInd_ind, bbbOpt]...
                    ...
                    if 1
                        ...
                        if 0
                            figure(FigBase + 20),subplot(figRow, figCol,1);hold on;plot(rad2deg(thetaSamp+ (k2cRef - k2cRef_00)), thetaProbInterp);grid on;
                            plot(rad2deg(angOpt4 - k2cRef_00), max(thetaProbInterp),'or');
                        else
                            if 1
                                figure(FigBase + 20),subplot(figRow, figCol,1);hold on;plot(rad2deg(thetaSamp+ (k2cRef - k2cRef_00)), thetaProb);grid on;title('1st and 2nd theta from p2 and z'); %title('2nd theta from p2 and z');
                                plot(rad2deg(angOpt4 - k2cRef_00), max(thetaProb),'or');
                            end
                            ...
                            %                              figure(FigBase + 20),
                            if 0
                                subplot(2,2,3);cla;imshow(imgCur);hold on;plot(pt2dCur(:,1), pt2dCur(:,2),'.r');title(num2str(size(pt2dCur,1)))
                            end
                    ...
                    if ~p2cAllFeat
                        twoOnlyP2List = [twoOnlyP2List; [[rad2deg([angOptList0' angOptList' angOptEpiInit k2cRef accumP2CTemp(end)] - [k2cRef_00.*ones(1,3 + 2*IterThetaNum)])] rad2deg(angOptList - accumP2CTemp(end))] ];
                    else
                        twoOnlyP2List = [twoOnlyP2List; [[rad2deg([angOptList0' angOptList' angOptEpiInit k2cRef k2cRef_pnp(end)] - [k2cRef_00.*ones(1,3 + 2*IterThetaNum)])] rad2deg(angOptList - k2cRef_pnp(end))] ];
                    end
                    figure(FigBase + 20);subplot(figRow, figCol,2);cla;hold on;
                    plot(twoOnlyP2List(:,end-2),'-sg');
                    colorCell0 = {};
                    colorCell0{1} = '-rx';
                    colorCell0{2} = '-mx';
...
%                         plot(twoOnlyP2List(:,end),'-om');
                    ...
                    plot(twoOnlyP2List(:,end-1),'-om');
                    plot(twoOnlyP2List(:,end),'-xc');
                    %                     legend('k2cRef - k2cRef0','1st theta from p2 and z','2nd theta', 'epipolar line', 'accum p2c','Location','southwest');title('ang - ref (deg)');
                    if forceAngFlag
                        %                     plot(size(twoOnlyP2List,1),twoOnlyP2List(end,IterThetaNum+uy),'*k');
                        plot(size(twoOnlyP2List,1),twoOnlyP2List(end,1+1),'*k','LineWidth',4);
                    end
                    if ~p2cAllFeat
                        title(sprintf('blue end err: % 0.8f\ndirect - accum: %0.4f', twoOnlyP2List(end,IterThetaNum+uy), rad2deg(angOptList - accumP2CTemp(end))));
                    else
                        title(sprintf('blue end err: % 0.8f\ndirect - accum: %0.4f', twoOnlyP2List(end,IterThetaNum+uy), rad2deg(angOptList - k2cRef_pnp(end))));
                    end
                    if 0
                        legend('k2cRef - k2cRef0','1st theta from p2 and z','2nd theta', 'accum p2c','Location','southwest');title('ang - ref (deg)');
                    end
                    if 0
                ...
                angRng = [thetaRng(ind1)  thetaRng(ind2);] + 0.1.*deg2rad((rand(1) - 0.5));
...
%                     REFAngList = [REFAngList; (k2cRef_00)];
                ...
                dispOfst = sum(repmat(dispStepRngTemp,length(inlierId),1)'.*probDispMat22Sum_1')';
                ...
                if 1
                    if 0 
                        figure(FigBase + 20),subplot(figRow, figCol,5);cla;plot(disparityRng, sum(ProbZTmp_update_norm_sum));hold on;plot(disparityRng,sum(ProbZTmp_update_norm_sum_tmp));legend('refined disp','gt disp');
                        ...
                        figure(FigBase + 20),subplot(figRow, figCol,5);cla;plot(dispStepRngTemp, sum(ProbZTmp_update_norm_sum));hold on;plot(dispStepRngTemp,sum(ProbZTmp_update_norm_sum_tmp));
                        if 0
                            legend('refined disp','gt disp');
                        end
%                         subplot(figRow, figCol,3);cla;plot(dispStepRngTemp, sum(ProbZTmp_update_norm_sum));hold on;plot(dispStepRngTemp,sum(ProbZTmp_update_norm_sum_tmp));
                        ProbZTmpTmp_update_norm__1 = ProbZTmpTmp_update_norm;
                        ProbZTmpTmp_update_norm__1(isnan(ProbZTmpTmp_update_norm__1)) = 0;
                        subplot(figRow,figCol,4);cla;imshow(imgCur);hold on;plot(pt2dCur(:,1), pt2dCur(:,2),'.r');title(strcat('red is good: ',num2str(size(pt2dCur,1)))); 
                        goodRatio = 0.80;
                        
                        if keyFrameLen == 2
                            data1 = [dispStepRngTemp;sum(ProbZTmp_update_norm_sum_tmp)];
                            save(fullfile(probPath,sprintf('dispDistrbution_%05d.mat',length(dir(fullfile(probPath,'dispDistrbution_*.mat')))+1)), 'data1');
                        end
                        
                        try
                            subplot(figRow,figCol,4);cla;imshow(imgCur);hold on;plot(pt2dCur(ProbZTmpTmp_update_norm__1 > goodRatio,1), pt2dCur(ProbZTmpTmp_update_norm__1 > goodRatio,2),'.r');
                            plot(pt2dCur(ProbZTmpTmp_update_norm__1 <= goodRatio,1), pt2dCur(ProbZTmpTmp_update_norm__1 <= goodRatio,2),'.g');
                            plot(pt2dCur(featInd,1),pt2dCur(featInd,2),'*b');
                            title(strcat('red is good: ',num2str(size(pt2dCur,1))));
                            try
                                subplot(figRow, figCol,3);cla;
                                plot(pt2dCur(ProbZTmpTmp_update_norm__1 > goodRatio,1) - pixGT(ProbZTmpTmp_update_norm__1 > goodRatio,1),pt2dCur(ProbZTmpTmp_update_norm__1 > goodRatio,2) - pixGT(ProbZTmpTmp_update_norm__1 > goodRatio,2),'+r');hold on;
                                plot(pt2dCur(ProbZTmpTmp_update_norm__1 <= goodRatio,1) - pixGT(ProbZTmpTmp_update_norm__1 <= goodRatio,1),pt2dCur(ProbZTmpTmp_update_norm__1 <= goodRatio,2) - pixGT(ProbZTmpTmp_update_norm__1 <= goodRatio,2),'sg');axis equal;
                                title(sprintf('LK - gt, err: [%0.3f %0.3f]',mean(pt2dCur(:,1) - pixGT(:,1)), mean(pt2dCur(:,2) - pixGT(:,2))));
                                try
                                    if 0
                                        subplot(figRow, figCol,6);cla;hist(depthGTInd_update(ProbZTmpTmp_update_norm__1 > goodRatio),100);hold on;hist(depthGTInd_update(ProbZTmpTmp_update_norm__1 <= goodRatio),100);
                                        subplot(figRow, figCol,6);cla;hist(dispStepRngTemp(depthGTInd_update(ProbZTmpTmp_update_norm__1 > goodRatio)),100);hold on;
                                        hist(dispStepRngTemp(depthGTInd_update(ProbZTmpTmp_update_norm__1 <= goodRatio)),100);
                                    else
                                        %                                         subplot(figRow, figCol,6);cla;hist(depthGTInd_update(ProbZTmpTmp_update_norm__1 > goodRatio),100);hold on;hist(depthGTInd_update(ProbZTmpTmp_update_norm__1 <= goodRatio),100);
                                        subplot(figRow, figCol,6);cla;plot(sort(dispStepRngTemp(depthGTInd_update(ProbZTmpTmp_update_norm__1 > goodRatio))),'-xr');hold on;
                                        plot(sort(dispStepRngTemp(depthGTInd_update(ProbZTmpTmp_update_norm__1 <= goodRatio))),'-xg');
                                    end
                                    %                                     depthGTInd_update
                                catch
                                    sakju = 1;
...
%                         drawnow;
                        if 0
                            saveas(gcf,fullfile(probPath,sprintf(strcat('depthDistri_',probDir(end-14:end),'____',sprintf('%04d',sum(KeyFrameFlagList)),'__%05d.png'),length(dir(fullfile(probPath,strcat('depthDistri_',probDir(end-14:end),'____',sprintf('%04d',(sum(KeyFrameFlagList))),'__*.png'))))+1 + FigBase)));
                        elseif 0
                            saveas(gcf,fullfile(probPath,sprintf(strcat('depthDistri_',probDir(end-14:end),'____',sprintf('%04d',sum(traceBatch)),'__%05d.png'),length(dir(fullfile(probPath,strcat('depthDistri_',probDir(end-14:end),'____',sprintf('%04d',(sum(traceBatch))),'__*.png'))))+1 + FigBase)));
                        else
                            asgkjh = 1;
                        end
                    end
            ...
            try
                sgdbjk = err_check(commonIdCheck, :) - trackingError(commonIdLocal, :);
            ...
            if UPDATEDEPTH
                ...
                elseif 0
                    ...
                    if 0
                        if 0
                            LocalTrace.probP2 = [LocalTrace.probP2; [{ProbReprojVecAllInit} {projDispMatAllInit}]];
                        else
                            LocalTrace.probP2 = [LocalTrace.probP2; [{ProbReprojVecAllInit(angSampInd,:,:)} {projDispMatAllInit(angSampInd,:,:)}]];
                        end
                        assert(sum(isnan(ProbReprojVecAllInit(:))) == 0);
                        
                        probP2Mat = []; probP2Mat2 = [];
                        if isempty(cfg.compensateFrmId)
                            startId = 1;
                        else
                            startId = cfg.compensateFrmId(1);
                    ...
                    figure(FigBase + 20),subplot(figRow, figCol, 5);plot(dispStepRngTemp, sum(probP2Mat22Sum_1));
                    hold on;plot(dispStepRngTemp, sum(ProbZTmp_update_norm_sum_tmp_p2c));plot(dispStepRngTemp, sum(ProbZTmp_update_norm_sum_tmp_c2c));
                    
                    pulse = dispStepRngTemp(1):0.001:dispStepRngTemp(end);
                    pulseVal = zeros(1,length(pulse));
                    [~, id_pulse] = min(abs(mean(dispOfst) - pulse));
                    pulseVal(id_pulse) = max(sum(ProbZTmp_update_norm_sum_tmp));
                    if 0
                        plot(pulse, pulseVal);
                    end
                    grid on;
                    title(sprintf('traceBatch: %d\nkeyFrameLen - 1: %d',traceBatch, keyFrameLen - 1));
                    
                    [~, idKeyDepth] = sort(LocalTrace.ptCcsZGT(:,1));
                    [~, idPrvDepth] = sort(LocalTrace.ptCcsZGT(:,end-1));
                    
                    
                    
                    errThr = 0;
                    pixer = pt2dCur(dispOfst>errThr,:);
                    pixer2 = pt2dCur(dispOfst<-errThr,:);
                    
                    [qa, qz] = hist(dispOfst);
                    if 0
                        dispOfstMean = sum(qa([1 end]).*qz([1 end]));
                    elseif 1
                        dispOfstMean = sum(qa([1 2 end-1 end]).*qz([1 2 end-1 end]));
                    elseif 0
                        dispOfstMean = sum(qz([1 end]));
                    else
                        varMinus = var(dispOfst(dispOfst < 0));
                        varPlus = var(dispOfst(dispOfst > 0));
                        if isnan(varPlus)
                            varPlus = 0;
                        end
                        if isnan(varMinus)
                        ...
                        if abs(varMinus - varPlus) <= 4.3483847e-04
                            dispOfstMean = 0;
                        end
                    ...
                    if 0
                        cfg.dispOfst = [cfg.dispOfst; -1];
                    else
                        cfg.dispOfst = [cfg.dispOfst; dispOfstMean];
                    end
                    figure(FigBase + 20),subplot(figRow, figCol, 7);cla; plot([(LocalTrace.ptCcsZ(idKeyDepth,1)) sort(LocalTrace.ptCcsZGT(:,1))]);title('key depth');
                                         subplot(figRow, figCol, 8);cla; plot([(LocalTrace.ptCcsZ(idPrvDepth,end-1)) sort(LocalTrace.ptCcsZGT(:,end-1))]);title('prv depth');
                    
                                         
                                         subplot(figRow, figCol, 9);hist(dispOfst);
                                         
                                         if figRow > 3
                                             
                                             subplot(figRow, figCol,10);imshow(dispMapCur_1, []);hold on;plot(pixer(:,1),pixer(:,2),'.r');plot(pixer2(:,1),pixer2(:,2),'.g');legend('>0','<0');
                                             
                                             subplot(figRow, figCol,11);plot([(LocalTrace.dispList(dispOfst>errThr,1) - LocalTrace.dispListGT(dispOfst>errThr,1))  (LocalTrace.dispList(dispOfst>errThr,end) - LocalTrace.dispListGT(dispOfst>errThr,end))]);title('>0   (stereo - gt)');legend('key','cur');
                                             subplot(figRow, figCol,12);plot([(LocalTrace.dispList(dispOfst<-errThr,1) - LocalTrace.dispListGT(dispOfst<-errThr,1))  (LocalTrace.dispList(dispOfst<-errThr,end) - LocalTrace.dispListGT(dispOfst<-errThr,end))]);title('<0   (stereo - gt)');legend('key','cur');
                                         end
                                         
                                         
                    figure(FigBase + 20),subplot(figRow, figCol, 2);  plot(0.05.*(sign(cfg.dispOfst)),'-xk');  
                    
                    figure(FigBase + 20),subplot(figRow, figCol, 5);
                    
                    pulse2 = dispStepRngTemp(1):0.001:dispStepRngTemp(end);
                    pulseVal2 = zeros(1,length(pulse2));
                    [~, id_pulse2] = min(abs(dispOfstMean - pulse2));
                    pulseVal2(id_pulse2) = max(sum(ProbZTmp_update_norm_sum_tmp));
                    if 1
                        plot(pulse2, pulseVal2, '-c');
                    end
                    ...
                    if 0
                        ...
                        try
                            saveas(gcf,fullfile(probPath,sprintf(strcat('depthDistri_',probDir(end-14:end),'____',sprintf('%04d__%06d',sum(traceBatch), length(obj.keyFrameFlagList)),'__%05d.png'),length(dir(fullfile(probPath,strcat('depthDistri_',probDir(end-14:end),'____',sprintf('%04d',(sum(traceBatch))),'__*.png'))))+1 + FigBase)));
                        catch
                            saveas(gcf,fullfile(probPath,sprintf(strcat('depthDistri_',probDir(end-14:end),'____',sprintf('%04d__%06d',sum(traceBatch), length(obj.keyFrameFlagList)),'__%05d.png'),length(dir(fullfile(probPath,strcat('depthDistri_',probDir(end-14:end),'____',sprintf('%04d',(sum(traceBatch))),'__*.png'))))+1 + FigBase)));
                        end
                    end
                    
                    frmNumSaved = length(dir(fullfile(probPath,strcat('depthDistri_',probDir(end-14:end),'____',sprintf('%04d',(sum(traceBatch))),'__*.png'))))+1;
                    
                    
                    
                    
                    
                    [pix_KinC, transXYZ_KinC] = VisualLocalizer.GetGtTrace2(b2cPmat, k2cRef_00, [LocalTrace0.ptIcsX(:,1) LocalTrace0.ptIcsY(:,1)],LocalTrace0.ptCcsZ(:,1),intrMat);
                    
                    [pix_KinC_gt, transXYZ_KinC_gt] = VisualLocalizer.GetGtTrace2(b2cPmat, k2cRef_00, [LocalTrace.ptIcsX(:,1) LocalTrace.ptIcsY(:,1)],LocalTrace.ptCcsZGT(:,1),intrMat);
                    
                    [pix_CinK, transXYZ_CinK] = VisualLocalizer.GetGtTrace2(b2cPmat, -angOptList, [LocalTrace.ptIcsX(:,end) LocalTrace.ptIcsY(:,end)],LocalTrace.ptCcsZ(:,end),intrMat);
                    
                    [pix_CinK_KinC, transXYZ_CinK_KinC] = VisualLocalizer.GetGtTrace2(b2cPmat, angOptList, [LocalTrace.ptIcsX(:,1) LocalTrace.ptIcsY(:,1)],transXYZ_CinK(:,3),intrMat);
                    if keyFrameLen > 2
                        if replaceKeyDisp2
                            [pix_CinK_KinC, transXYZ_CinK_KinC_replace] = VisualLocalizer.GetGtTrace2(b2cPmat, angOptList, [LocalTrace.ptIcsX(:,1) LocalTrace.ptIcsY(:,1)],LocalTrace.ptCcsZ(:,1),intrMat);
                        else
                            [pix_CinK_KinC, transXYZ_CinK_KinC_replace] = VisualLocalizer.GetGtTrace2(b2cPmat, angOptList, [LocalTrace.ptIcsX(:,1) LocalTrace.ptIcsY(:,1)],keyDispWeighted_bak',intrMat);
                        end
                    else
                        [pix_CinK_KinC, transXYZ_CinK_KinC_replace] = VisualLocalizer.GetGtTrace2(b2cPmat, angOptList, [LocalTrace.ptIcsX(:,1) LocalTrace.ptIcsY(:,1)],LocalTrace.ptCcsZ(:,1),intrMat);
                    ...
                    transDisp_CinK = (intrMat(1,1).*norm(obj.camModel.transVec1To2))./(transXYZ_CinK(:,3) + (princpPtR(1) - princpPtL(1)));
                    
                    [xyz_CinC] = GetXYZFromDepth(intrMat, pt2dCur,  LocalTrace.ptCcsZ(:,end));
                    
                    
                    
%                     if 0
                        dispe_cur = LocalTrace0.dispList(:,end) - LocalTrace.dispListGT(:,end);
%                     else
                        dispe_key = LocalTrace0.dispList(:,1) - LocalTrace.dispListGT(:,1);
%                     end
                    
                    if isempty(LocalTrace.transDisp)
                         [xyz_KinK] = GetXYZFromDepth(intrMat, Pix,  LocalTrace0.ptCcsZ(:,1));
                         [xyz_KinK_gt] = GetXYZFromDepth(intrMat, Pix,  LocalTrace.ptCcsZGT(:,1));
%                          [xyz_CinC_gt] = GetXYZFromDepth(intrMat, Pix,  LocalTrace.ptCcsZGT(:,1));
                         dispe_cur0 = LocalTrace0.dispList(:,1) - LocalTrace.dispListGT(:,1);
%                         LocalTrace.transDisp = [LocalTrace.transDisp; [{xyz_KinK'} {xyz_KinK'} {single(LocalTrace.dispList(:,1)')} {xyz_KinK_gt'} {xyz_KinK'}         {single(zeros(1, length(single(LocalTrace.dispList(:,1)'))))} {single(zeros(1, length(single(LocalTrace.dispList(:,1)'))))}     {single(dispe_cur0')}    {LocalTrace.ptCcsZGT(:,1)'} {xyz_KinK_gt(:,3)' - xyz_KinK(:,3)'} {zeros(1,size(xyz_KinK,1))}]];
                        LocalTrace.transDisp = [LocalTrace.transDisp; [{xyz_KinK'} {xyz_KinK'} {single(LocalTrace.dispList(:,1)')} {xyz_KinK_gt'} {xyz_KinK'}         {single(dispe_cur0')}   {single(dispe_cur0')}    {single(dispe_cur0')}    {LocalTrace.ptCcsZGT(:,1)'} {xyz_KinK_gt(:,3)' - xyz_KinK(:,3)'} {zeros(1,size(xyz_KinK,1))}]];
                    end
                        
                    if 0
                        LocalTrace.transDisp = [LocalTrace.transDisp; [{transXYZ_KinC'} {transXYZ_CinK_KinC'} {transDisp_CinK'} {transXYZ_KinC_gt'} {xyz_CinC'}        {single(transDisp_CinK' - LocalTrace0.dispList(:,1)')}  {single(LocalTrace.dispList(:,1)' - LocalTrace0.dispList(:,1)')}     {single(dispe_cur')}      {LocalTrace.ptCcsZGT(:,1)'} {[]} {[]}]];
                    elseif 0
                        LocalTrace.transDisp = [LocalTrace.transDisp; [{transXYZ_KinC'} {transXYZ_CinK_KinC_replace'} {transDisp_CinK'} {transXYZ_KinC_gt'} {xyz_CinC'}         {single(transDisp_CinK' - LocalTrace0.dispList(:,1)')}    {single(LocalTrace.dispList(:,1)' - LocalTrace0.dispList(:,1)')}     {single(dispe_cur')}     {LocalTrace.ptCcsZGT(:,1)'} {[]} {[]}]];
                    else
                        %                         LocalTrace.transDisp = [LocalTrace.transDisp; [{transXYZ_KinC'} {transXYZ_CinK_KinC_replace'} {transDisp_CinK'} {transXYZ_KinC_gt'} {xyz_CinC'}         {single(transDisp_CinK' - LocalTrace0.dispListGT(:,1)')}    {single(LocalTrace.dispList(:,1)' - LocalTrace0.dispListGT(:,1)')}     {single(dispe_key')}     {LocalTrace.ptCcsZGT(:,1)'} {[]} {[]}]];
                        if keyFrameLen > 2
                            if replaceKeyDisp2
                                LocalTrace.transDisp = [LocalTrace.transDisp; [{transXYZ_KinC'} {transXYZ_CinK_KinC'} {transDisp_CinK'} {transXYZ_KinC_gt'} {xyz_CinC'}         {single(transDisp_CinK' - LocalTrace0.dispListGT(:,1)')}    {single(LocalTrace.dispList(:,1)' - LocalTrace0.dispListGT(:,1)')}     {single(dispe_key')}     {LocalTrace.ptCcsZGT(:,1)'} {[]} {[]}]];
                            else
                                LocalTrace.transDisp = [LocalTrace.transDisp; [{transXYZ_KinC'} {transXYZ_CinK_KinC'} {transDisp_CinK'} {transXYZ_KinC_gt'} {xyz_CinC'}         {single(transDisp_CinK' - LocalTrace0.dispListGT(:,1)')}    {single(keyDispWeighted_bak - LocalTrace0.dispListGT(:,1)')}     {single(dispe_key')}     {LocalTrace.ptCcsZGT(:,1)'} {[]} {[]}]];
                            end
                        else
                            LocalTrace.transDisp = [LocalTrace.transDisp; [{transXYZ_KinC'} {transXYZ_CinK_KinC'} {transDisp_CinK'} {transXYZ_KinC_gt'} {xyz_CinC'}         {single(transDisp_CinK' - LocalTrace0.dispListGT(:,1)')}    {single(LocalTrace.dispList(:,1)' - LocalTrace0.dispListGT(:,1)')}     {single(dispe_key')}     {LocalTrace.ptCcsZGT(:,1)'} {[]} {[]}]];
                        end
                    ...
                    AA = cell2mat(LocalTrace.transDisp(:,1));
                    AAA = AA(3:3:end,:);
                    zzzzz = AA(3:3:end,:);
                    yyyyy = AA(2:3:end,:);
                    xxxxx = AA(1:3:end,:);
                    
                    BB = cell2mat(LocalTrace.transDisp(:,2));
                    BBB = BB(3:3:end,:);
                    zzzzz2 = BB(3:3:end,:);
                    yyyyy2 = BB(2:3:end,:);
                    xxxxx2 = BB(1:3:end,:);
                    
                    CC = cell2mat(LocalTrace.transDisp(:,4));
                    CCC = CC(3:3:end,:);
                    zzzzz3 = CC(3:3:end,:);
                    yyyyy3 = CC(2:3:end,:);
                    xxxxx3 = CC(1:3:end,:);
                    
                    DD = cell2mat(LocalTrace.transDisp(:,5));
                    DDD = DD(3:3:end,:);
                    zzzzz4 = DD(3:3:end,:);
                    yyyyy4 = DD(2:3:end,:);
                    xxxxx4 = DD(1:3:end,:);
                    
                    dispe = LocalTrace0.dispList(:,1) - LocalTrace.dispListGT(:,1);
                    
                    LocalTrace.transDisp{end,end-1} = zzzzz3 - zzzzz; % gtK - stereoK;
                    LocalTrace.transDisp{end,end} = zzzzz2 - zzzzz; % stereoC - stereoK
                    ...
                    if 0
                        if 1
                            pickFeat = 10;
                            figure,subplot(2,2,1);plot(dispe);title('stereo - gt');
                            subplot(2,2,2),plot(xxxxx(:,pickFeat), zzzzz(:,pickFeat),'-+r');hold on;
                            plot(xxxxx2(:,pickFeat),  zzzzz2(:,pickFeat),'-+b');
                            plot(xxxxx3(:,pickFeat),  zzzzz3(:,pickFeat),'-+g');
                            plot(xxxxx4(:,pickFeat),  zzzzz4(:,pickFeat),'-+c');
                            legend('k','c','gt','cc');title(sprintf('featId: %d\nstereo - gt: %0.3f',pickFeat,dispe(pickFeat)));
                            axis equal;xlabel('x');ylabel('z');
                            subplot(2,2,3),hist(sum(LocalTrace.transDisp{end,end-1})); title('gtK - stereoK');
                            subplot(2,2,4),hist(sum(LocalTrace.transDisp{end,end})); title('stereoC - stereoK');
                        else
                            figure,plot3(xxxxx(:,pickFeat), yyyyy(:,pickFeat), zzzzz(:,pickFeat),'-+r');hold on
                            plot3(xxxxx2(:,pickFeat), yyyyy2(:,pickFeat), zzzzz2(:,pickFeat),'-+b');
...
%                     LocalTrace.probP2 = [LocalTrace.probP2; {ProbReprojVecAllInit}];
            ...
            bbbOpt = bbbOpt2;
            if ~isfield(cfg, 'bbbOpt')
                cfg.bbbOpt = {};
                % % %                 cfg.angSampInd_gt = [];
            end
            cfg.bbbOpt = [cfg.bbbOpt; {bbbOpt}];
            
            
            
            inTrackFlag0011 = inlierId > 0;
            pnp_ang_est_max_margin_2 = [deg2rad([-0.1 0.1]) 2];
            [k2cBodyRotAng_2,Err_2,inId] = VisualLocalizer.RotationAngleEstimate_PNP_(pnp_ang_est_max_margin_2,intrMat,Pix,pt2dCur,LocalTrace.ptCcsZ(:,1), find(inTrackFlag0011),true(sum(inTrackFlag0011),1),b2c,double(angOpt), 0.000001);
            
            
            if 0
                for yh = 1 : length(inTrackFlag0011)
                    [k2cBodyRotAng_21(yh,1),~,~] = VisualLocalizer.RotationAngleEstimate_PNP_(pnp_ang_est_max_margin_2,intrMat,Pix(yh,:),pt2dCur(yh,:),LocalTrace.ptCcsZ(yh,1), find(inTrackFlag0011(yh)),true(sum(inTrackFlag0011(yh)),1),b2c,double(bbbOpt2(yh)), 0.000001);
                end
                figure,plot(rad2deg(k2cBodyRotAng_21 - bbbOpt2'));
                rad2deg(angOpt - mean(bbbOpt))
            end
            
            try
                [u7, u8] = hist(bbbOpt,20);
                
                idoo = find(u7 > 0.*0.05.*length(bbbOpt)/length(u8));
                idooRng = [u8(idoo(2)) u8(idoo(end-1))];
                
                id00 = find(thetaProb./max(thetaProb) > 0.8);
                id00Rng = [thetaRng(id00(1)) thetaRng(id00(end))];
                
                if 1
                    idooo = find(bbbOpt < idooRng(1) | bbbOpt > idooRng(2));
                else
                    idooo = find(bbbOpt < id00Rng(1) | bbbOpt > id00Rng(2));
                end
...
% %                 [k2cBodyRotAng_2,Err_2,inId] = VisualLocalizer.RotationAngleEstimate_PNP_(pnp_ang_est_max_margin_2,intrMat,Pix,pt2dCur,LocalTrace.ptCcsZ(:,1), find(inTrackFlag0011),true(sum(inTrackFlag0011),1),b2c,double(angOpt), 0.000001);
                ...
                if keyFrameLen < outIdFrameThr
                    LocalTrace.featIdOut = LocalTrace.featId(idooo);
                else
                    LocalTrace.featIdOut = [];
                end
            catch
                LocalTrace.featIdOut = [];
            end
            ...
            if 0
                LocalTrace.featIdOut =  LocalTrace.featId(KKKK_unValid);
            end
            
            
            if isfield(cfg, 'newAngOpt')    
                angOptNew = cfg.newAngOpt(1:keyFrameLen - 1);
                
                if ~isfield(cfg, 'angOptErr')
                    cfg.angOptErr = [[angOptNew(end)  angOptList(end)]];
                else
                    cfg.angOptErr = [cfg.angOptErr; [angOptNew(end)  angOptList(end)]];
                end
                
                
                if 0
%                     figure,plot(diff(rad2deg([[0 0];cfg.angOptErr])));
                    figure,plot(rad2deg(diff([0;cfg.angOptErr(:,1)]) - diff([0;cfg.angOptErr(:,2)])))
                end
                
            end
            
            % %             angOptList(end) = k2cBodyRotAng_2;
            
            %             if ~isfield(cfg, 'pnpErr')
            if keyFrameLen == 2
                cfg.pnpErr = [angOptList k2cBodyRotAng_2];
            else
                if ~isfield(cfg, 'pnpErr')
                    cfg.pnpErr = [angOptList k2cBodyRotAng_2];
                else
                    cfg.pnpErr = [cfg.pnpErr; [angOptList k2cBodyRotAng_2]];
                end
                
                
                if 0
                    figure,plot(rad2deg(cfg.pnpErr(:,2) - cfg.pnpErr(:,1)))
                end
...
%                 
%             end
            if isfield(cfg, 'LocalTrace10')    
                LocalTrace10 = cfg.LocalTrace10;
                ...
                try
                    if ~isfield(cfg, 'angOptErr')
                        rrrrr
                        fd = 2; ParticularFeatUpdate(1, LocalTrace, fd, depthGTInd_update,disparityRng, fd, curUpdateRatio, 1,cfg.angSampInd_gt)
                    else
                        fd = 2; ParticularFeatUpdate(1, LocalTrace, fd, depthGTInd_update,disparityRng, fd, curUpdateRatio, 1,[])
                    end
                catch
            ...
            if 0
                figure,plot(trackingError);hold on;plot(idooo,trackingError(idooo),'or');
                figure,plot(Err_2);
                v= 2943;[LocalTrace.ptIcsX(v,1) LocalTrace.ptIcsY(v,1) LocalTrace.ptIcsX(v,end) LocalTrace.ptIcsY(v,end) LocalTrace.dispList(v,:) LocalTrace.dispListGT(v,:)]
                fd = 2; ParticularFeatUpdate(1, LocalTrace, fd, depthGTInd_update,disparityRng, fd, curUpdateRatio, 1,cfg.angSampInd_gt)
                fd = 2; featId2check = fd; FeatTrackingError
                errOldKey = ParticularFeatDirectVSAccum(sc, obj, LocalTrace10, [fd;fd], [obj.accumP2CRef(end-(keyFrameLen-1):end) - obj.accumP2CRef(end-(keyFrameLen-1))], angSampRng, curUpdateRatio, depthGTInd_update, [], 0,0);
                errNewKey = ParticularFeatDirectVSAccum(sc, obj, LocalTrace, [fd;fd], [obj.accumP2CRef(end-(keyFrameLen-1):end) - obj.accumP2CRef(end-(keyFrameLen-1))], angSampRng, curUpdateRatio, depthGTInd_update, [], 0,0);
                errGtKey = ParticularFeatDirectVSAccum(sc, obj, LocalTrace, [fd;fd], [obj.accumP2CRef(end-(keyFrameLen-1):end) - obj.accumP2CRef(end-(keyFrameLen-1))], angSampRng, curUpdateRatio, depthGTInd_update, [], 0,1);
                figure,plot([errOldKey(:,1) - errGtKey(:,1) errNewKey(:,1) - errGtKey(:,1)]);legend('old - gt','new - gt');
                figure,plot([(errOldKey(:,1) - errGtKey(:,1)) - (errNewKey(:,1) - errGtKey(:,1))]);legend('old - gt','new - gt');
                figure,plot([errOldKey(:,1)  errNewKey(:,1)]);legend('old - gt','new - gt');
                figure,plot([errOldKey(:,1) - errNewKey(:,1)]);
                
                errNewKey = [[0 0];errNewKey];
                errGtKey = [[0 0];errGtKey];
                figure,plot([diff(errNewKey(:,1)) - diff(errGtKey(:,1))] )
...
%             pixer2 = pt2dCur(dispOfst<-errThr,:);
            ...
            if 0
                figure,subplot(1,3,1);imshow(dispMapCur_1, []);hold on;plot(pixer(:,1),pixer(:,2),'.r');plot(pixer2(:,1),pixer2(:,2),'.g');legend('>0','<0');
                subplot(1,3,3);plot([(LocalTrace.dispList(dispOfst<-errThr,1) - LocalTrace.dispListGT(dispOfst<-errThr,1))  (LocalTrace.dispList(dispOfst<-errThr,end) - LocalTrace.dispListGT(dispOfst<-errThr,end))]);title('<0   (stereo - gt)');
                subplot(1,3,2);plot([(LocalTrace.dispList(dispOfst>errThr,1) - LocalTrace.dispListGT(dispOfst>errThr,1))  (LocalTrace.dispList(dispOfst>errThr,end) - LocalTrace.dispListGT(dispOfst>errThr,end))]);title('>0   (stereo - gt)');
            end
...
depthListGT = LocalTrace.ptCcsZGT(:,1);
