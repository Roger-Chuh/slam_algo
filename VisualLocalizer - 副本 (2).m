...
%                         if 1
                            ...
                            if DRAWPOLYGON
                                if ~isempty(inlierId)
                                    ...
                                    if 1
                                        try
                                            [angOpt1, inlierIdNew,  angRng1, pixKey ,pt2dCur, DispRefine, angOpt33, inFlag] = RefineZ(obj, k2cRef,k2cBodyRotAng, inlierId,inlierId2, theta, DepthProbility,  DepthId,  AngleModalOrg,  AngleProbility,prevFeatPtList0,ptCcsZ0,DispRefineUse__);  %, keyCcsXYZAll);
...
%           function [angOpt, inlierId, angRng, PixUse, pt2dCurUse, DispRefineUse, angOpt3, inFlag] = RefineZ(obj, k2cRef,k2cPnp, inlierId0, inlierId2, theta, DepthProbility,  DepthId,  AngleModalOrg,  AngleProbility,prevFeatPtList0,ptCcsZ0)  %, XYZ)
          function [angOpt, inlierId, angRng, PixUse, pt2dCurUse, DispRefineUse_, angOpt3, inFlag] = RefineZ(obj, k2cRef,k2cPnp, inlierId0, inlierId2, theta, DepthProbility,  DepthId,  AngleModalOrg,  AngleProbility,prevFeatPtList0,ptCcsZ0,DispRefineUse__)  %, XYZ)
            global SHOWPOLYGON DRAWPOLYGON ANGLEONLY USEGOLDENDISP UPDATEDEPTH DEPTHITERNUM probPath ...
                ...
                doRoundingTheta doRoundingRefAngList3
...
%                                     plot(rad2deg([optAng(1:end);thetaRng(idM)]) - rad2deg(obj.refAngList),'-xk');
                            ...
                            [depthGTInd_update, ProbZTmpTmp_update_norm, ProbZTmp_update_norm, idMax_update, depthGTIndAll_update, depthUpdateIndAll_update, ProbZNormSum] = VisualLocalizer.GetDepthHist(updatedProbZSum_,disparityErrorRound,obj.configParam.disparity_sample_step);
                            ...
                            DispRefineUse_ = sum(DispRngUse'.*ProbZNormSum')';
...
%                             doProb = 1; idM_Vec = -20:20; debugProb;
    ...
    methods (Static)
        function [depthGTInd, ProbZTmpTmpNormUse, ProbZTmpNorm, peakIdTmp, depthGTIndAll, depthUpdateIndAllUse, ProbZNormSum] = GetDepthHist(ProbZ,disparityErrorRound,disparity_sample_step)
...
%             obj.modals = m_;
    ...
    methods (Static)
        function [curPredPtIcs,inTraceFlag,curPredPtIcs0,inTraceFlag0,weightRaw,candX,candY] = NewTracking(obj,keyFeatNum,prevFeatPtList,ptCcsZ,intrMat,k2cRef,angleModalOrg, keyDepthV, prevDepthV, currDepthV,DepthProbility, DepthId, AngleProbility,reCalc,DispRefineUse__)
            global doRoundingTheta
...
%             obj.modals = m_;
    ...
    methods (Static)
        function [curPredPtIcs,inTraceFlag,curPredPtIcs0,inTraceFlag0,weightRaw,candX,candY] = NewTrackingTemp(obj,keyFeatNum,prevFeatPtList,ptCcsZ,intrMat,k2cRef,angleModalOrg, keyDepthV, prevDepthV, currDepthV,DepthProbility, DepthId, AngleProbility,reCalc,DispRng,DispRefineUse__)
            global doRoundingTheta
...
%                 angleOrgin = rad2deg(k2cRef - obj.refAngList3(end));
            ...
            if 0
                ...
                obj.setting.configParam.disparityOrginRange   = DispRng; % DepthProbility0;  %existPtDisparityG + obj.setting.configParam.disparityRange;
            end
%             table = LookUpTables.LookUpTable(obj.lookUpTables,existFeaturesNumber,obj.setting.configParam.tableSize); %initialize
            table = LookUpTables.LookUpTable(existFeaturesNumber,obj.setting.configParam.tableSize); %initialize
            table.winSize = obj.lookUpTables.winSize;
