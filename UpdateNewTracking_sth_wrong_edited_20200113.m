function [thetaProbUse, thetaProbUse_, thetaProbUse__, curPredPtIcs, weightRaw, candX, candY,candX_reshape, candY_reshape, weightRaw_reshape,idff,errPair,PixReprojExpXY,objPervPtCcsZGolden1,p2cErrList] = ...
    UpdateNewTracking_sth_wrong_edited_20200113(obj, thetaSamp, thetaProb, prevFeatPtList0, ptCcsZ0, intrMat, AngleModalOrg, DepthProbility, DepthId, inlierId, DispRng,DispRefineUse__, angOptP2C,DispKeyInPrvRng,angOpt,thetaRng, thetaProbP2C, k2cRef,k2cRef0,thetaRngP2C,thetaSampP2C,thetaProbP2CBeforeNewPlatform,thetaProbBeforeNewPlatform,thetaRngP2CBeforeNewPlatform,thetaRngBeforeNewPlatform,thetaP2C,thetaP2C0,...
        ...
        if 1
            if 1 % ~useK2C
                if ~EXPZERO
                    ...
                    angleMiddle =  k2cRef0 - obj.refAngList(end);
                end
                
                if UseGoldenThetaP2C
                    angleMiddle =  k2cRef0 - obj.refAngList(end);thetaP2C;
                end
...
inlierId0 = [1:length(inlierId)]';



iff = (tempPt(:,1) ~= -1);
idff = find(ismember(find(iff),inlierId0));

iff(setdiff(1:size(tempPt,1),inlierId0)) = false;
tempPt2 = tempPt(iff,:);
