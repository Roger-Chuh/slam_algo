function CalibB2C5_bak(inputDir, startt, endd)
    ...
    if length(featIdKey) > numThr
        ...
        if 1 %~forceFixesAxis
            ...
            for j = 1 : minFrameNum - 1
                ...
                xyzCur2 = RTTemp*pextend(xyzKey');
                xyzCur2 = xyzCur2(1:3,:)';
                [~, xyzErr] = NormalizeVector(xyzCur2 - xyzCur);
            ...
            axisVec0 = [pi/2;pi/2];
            err0 = FixAxisPnp(intrMat, thetaList, xyzKey, xMat, yMat, minFrameNum, RtTemp{cnt,1}, b2c, inlierIdComm, axisVec0);
            %         options = optimoptions('lsqnonlin','Algorithm','levenberg-marquardt','Display','off','MaxFunEvals',10000,'TolX',1e-8);%,'MaxFunEvals',10000);%, 'MaxIterations',5);
            lb = [(pi/2 - rad2deg(0.05)); 0];
            ub = [(pi/2 + rad2deg(0.05)); 2*pi];
            
            if 0
                [vec,resnorm,residual,exitflag,output,lambda,jacobian] = lsqnonlin(@(X) FixAxisPnp(intrMat, thetaList, xyzKey, xMat, yMat, minFrameNum, RtTemp{cnt,1}, b2c,inlierIdComm, X),[axisVec0],lb,ub,options);%,data_1,obs_1)
            else
                [vec,resnorm,residual,exitflag,output,lambda,jacobian] = lsqnonlin(@(X) FixAxisPnp(intrMat, thetaList, xyzKey, xMat, yMat, minFrameNum, RtTemp{cnt,1}, b2c,inlierIdComm, X),[axisVec0],[],[],options);%,data_1,obs_1)
            end
            
            [err1, camPoseC2K_, camPoseK2C_, bodyPoseK2C_] = FixAxisPnp(intrMat, thetaList, xyzKey, xMat, yMat, minFrameNum, RtTemp{cnt,1}, b2c, inlierIdComm,vec);
...
if 1
    for ji = 1 : size(b2c_vec_norm,1)
        ...
        [sd(ji,:),fg(ji,:)] = hist(real(Ang2),100);
%         figure,hist(real(Ang2),100);
...
vecUse = Vec(:,bestId);
[initErr,ppp] = CostFunc(b2c, traceX, traceY, intrMat, minFrameNum, cnt-1, validCnt, goodId, thetaList, XYZKey, numThr, RtTemp, Inliers, vecUse);
[vecOpt,resnorm,residual,exitflag,output,lambda,jacobian] = lsqnonlin(@(X) CostFunc(b2c, traceX, traceY, intrMat, minFrameNum, cnt-1, validCnt, goodId, thetaList, XYZKey, numThr, RtTemp, Inliers,X),[vecUse],[],[],options);%,data_1,obs_1)

[optErr,ooo] = CostFunc(b2c, traceX, traceY, intrMat, minFrameNum, cnt-1, validCnt, goodId, thetaList, XYZKey, numThr, RtTemp, Inliers, vecOpt);
...
function varargout = CostFunc(b2c, traceX, traceY, intrMat, minFrameNum, cnt0, validCnt, goodId, thetaList, XYZKey, numThr, RtTemp, Inliers, rotAxisVec)
r = 1;
theta = rotAxisVec(1);
phi = rotAxisVec(2);
...
for kkk = 1 : 1 % length(goodId)
    ...
    for i = 1 : size(traceX,2) - (minFrameNum-1)
        if ismember(i, validCnt(goodId))
            ...
            if length(featIdKey) > numThr
                ...
                error = FixAxisPnp(intrMat, thetaList, xyzKey, xMat, yMat, minFrameNum, RtTemp{cnt,1}, b2c, Inliers{cnt,1}, rotAxisVec);
    ...
    if (nargout > 1)
        ...
        subplot(2,4,5);plot(-median(XErrMat(:,:)));title('x');
        subplot(2,4,6),plot(-median(YErrMat(:,:)));title('y');
