...
if 0
    ...
    ratioList = 0.6788667;  + [-0.01:0.001/3:0.01]; [0.77:-0.0002:0.73]; 0.2480;
    thetaList =  7.087;  + [-0.02:0.001/2:0.02]; 0;
end
dir1 = xz2(end,:) - xz2(1,:);
dir1 = dir1./norm(dir1);
cnt = 1;
...
img = imread(fullfile(inputDir, dirInfo(5481).name));
imggg = ImgTransform(rgb2gray(img), intrMat, R2');testBirdView2(intrMat,imggg)

imggg = ImgTransform(rgb2gray(img), intrMat, R0');testBirdView2(intrMat,imggg)
% figure,imshow(img1)
% figure,imshow(img2)
% figure,imshow(img11)
% figure,imshow(img22)
