function [DistAng1, DistAng2,angRangeInd,indAlignRange,DistAng] = getDistAngInfo(accumAng, camPoseMatNew, angThr,window,marginInd,minNum,tss,varargin)
% marginInd = 2;
...
if ~isempty(paraDir)
    param = load(fullfile(paraDir,'CornerHeight.txt'));
    % height = param(1);
    veloThrRobo = param(2);
    % veloThrCeil = param(3);
    window3Robo = param(4);
    % window3Ceil = param(5);
    minPeakNumRobo = param(6);
    % minPeakNumCeil = param(7);
    splitThrCeil = param(8);
    filtThrCeil = param(9);
    marginThrCeil = param(10);
    minNumThrCeil = param(11);
...
marginIndInd = 2;5;2;1;
if accumAng(end) < 0
    accumAng = -accumAng;
end

angThr2 = angThr + 0; %1; %0;  %1;
...
[segInfoRobo, MiniRobo, DistAng] = segPath([repmat(camPoseMatNew,1,4) tss accumAng], veloThrRobo, veloThrRobo, window3Robo,minPeakNumRobo,1,draw);
%     dltDistRobo = medfilt1(dltDistRobo1,(2*10 - 1));
[indexCellRoboNew112,idxRoboNew] = splitIndex2(find(dltDistRoboFiltTempNew > angThr));
[indexCellRoboNew2,idxRoboNew2] = splitIndex2(find(dltDistRoboFiltTempNew <= angThr2));


for v = 1 :length(indexCellRoboNew112)
    splitInd(v,:) = indexCellRoboNew112{v}([1 end]);
end
indErr = abs(splitInd(1:end-1,2)  - splitInd(2:end,1));
try
    [indexCellRoboNewSplit,idxRoboNewSplit] = splitIndex2(find(indErr <= minSplit));
catch
    indexCellRoboNewSplit{1} = [];
end
% % idSp = find(indErr > minSplit);

if ~isempty(indexCellRoboNewSplit{1})
    
    for v = 1 : length(indexCellRoboNewSplit)
        indexCellRoboNewSplit2{v,:} = [indexCellRoboNewSplit{v}(1):indexCellRoboNewSplit{v}(end)+1];
    end
    
    
    idSp = setdiff([1:size(splitInd,1)],cell2mat(indexCellRoboNewSplit2'));
    for vv = 1: length(idSp) + length(indexCellRoboNewSplit2)
        
        if vv <= length(idSp)
            splitIndexx(vv,:) = [splitInd(idSp(vv),:)];
        else
            tmpInd = splitInd(indexCellRoboNewSplit2{vv- length(idSp)},:);
            splitIndexx(vv,:) = [min(tmpInd(:)) max(tmpInd(:))];  %splitInd(indexCellRoboNewSplit2{vv- length(idSp)},:);
        end
        
    end
    
    [~,idMin] = sort(splitIndexx(:,1),'ascend');
    splitIndex = splitIndexx(idMin,:);
    for vvv = 1:size(splitIndex,1)
        indexCellRoboNew11{vvv,1} = [splitIndex(vvv,1):splitIndex(vvv,2)];
    end
else
    
    for vvv = 1:size(splitInd,1)
        indexCellRoboNew11{vvv,1} = [splitInd(vvv,1):splitInd(vvv,2)];
    end
    
end

% %
% % for v = 1 : size(indErr,1)
% %
% %
% % end
...
if 0
    ...
    % % end
    dltDistRoboFiltTempNew = dltDistRoboFiltTempNewAng;
    %     dltDistRobo = medfilt1(dltDistRobo1,(2*10 - 1));
    [indexCellRoboNew1111,idxRoboNew] = splitIndex2(find(dltDistRoboFiltTempNew > 1.5));
...
if draw == 1
    ...
    subplot(3,1,3);plot(dltDistRoboFiltTempNew);title(sprintf('after filter, angle threshold = %.3f', angThr));hold on;plot(idxAll,'r');hold on;plot(idxAll,'r');
end
...
for q = 1 : length(indexCellRoboNew)
    ...
    % %
    indRange1Ang = [indRange1(1) - marginInd - 0 min(indRange1(2) + marginInd + marginIndInd, size(camPoseMatNew,1))];% loose
    ...
    % %     indRange1Ang = [indRange1(1) - marginInd - marginIndInd min(indRange1(2) + marginInd + marginIndInd, size(camPoseMatNew,1))];% loose
    % %     indRange1Dist = [min(indRange1(1) + marginInd + marginIndInd, size(camPoseMatNew,1)) min(indRange1(2) - marginInd - marginIndInd, size(camPoseMatNew,1))];%tight
...
if 0
    ...
    for j = 1 : size(IndRange1,1) + 1
        ...
        try
            if 1 %q > 1
                ...
                d1 = abs(accumAng(indRange2Ang(2)) - accumAng(indRange2Ang(1)) - 90);
                d2 = abs(accumAng(indRange2Ang(2)) + accumAng(indRange2Ang(1)) - 90);
                if d1 < d2
                    DistAng2(j,2) = accumAng(indRange2Ang(2)) - accumAng(indRange2Ang(1));
                else
                    DistAng2(j,2) = accumAng(indRange2Ang(2)) + accumAng(indRange2Ang(1));
    ...
    if draw == 1
        figure,subplot(3,1,1);plot(accumAng,'-');title('ang');hold on;plot(indxAng, accumAng(indxAng),'*r');plot(indxDist, accumAng(indxDist),'*g'); legend('curve','ang','dist');%plot(idxAll3,'r');
        subplot(3,1,2);plot(camPoseMatNew(:,1));title('x');hold on;plot(indxAng, camPoseMatNew(indxAng,1),'*r');plot(indxDist, camPoseMatNew(indxDist,1),'*g');  legend('curve','ang','dist');%plot(idxAll4,'r');
        subplot(3,1,3);plot(camPoseMatNew(:,3));title('z');hold on;plot(indxAng, camPoseMatNew(indxAng,3),'*r'); plot(indxDist, camPoseMatNew(indxDist,3),'*g');legend('curve','ang','dist');%plot(idxAll5,'r');
        figure,plot(camPoseMatNew(:,1),camPoseMatNew(:,3),'-');hold on;plot(camPoseMatNew(indxAng,1),camPoseMatNew(indxAng,3),'*r');plot(camPoseMatNew(indxDist,1),camPoseMatNew(indxDist,3),'*g');axis equal;legend('curve','ang','dist');
    end
    
    indAlign = sort([indxDist(2:end-1);indxAng(2:end-1)],'ascend');
    
    if mod(length(indAlign),2) == 1
        indAlign = indAlign(1:end-1);
    end
    
    indAlignRange = [indAlign(1:2:end) indAlign(2:2:end)];
    indAlignRange(1:end-1,2) = indAlignRange(1:end-1,2) + 3;
    indAlignRange(1:end-1,1) = indAlignRange(1:end-1,1) - 3;
    if mod(size(indAlignRange,1),2) == 1
        indAlignRange = indAlignRange(1:end-1,:);
    end
    
    if draw == 1
        figure,plot(tss,accumAng,'-');title('ang');hold on;plot(tss(indAlignRange(:,1)), accumAng(indAlignRange(:,1)),'*r');plot(tss(indAlignRange(:,2)), accumAng(indAlignRange(:,2)),'*g'); legend('curve','ang','dist');%plot(idxAll3,'r');
    end
    ...
    for i = 1 : size(IndRange2,1)
        ...
        for ii = 1 : size(DistAngList,1)
            ...
            distInfo{i,1}(ii,:) = [coord(1:3,4)' DistAngList(ii,end)];
            
        end
        if i == 7
            asdvgk = 1;
        end
        distInfo{i}(:,end) = distInfo{i}(:,end) - distInfo{i}(1,end);
        distInfo{i} =  distInfo{i}( distInfo{i}(:,end) < 5,:);
        if max(abs(distInfo{i}(:,end))) < 3
            if draw == 1
                figure(32),hold on;plot(distInfo{i}(:,3),distInfo{i}(:,4));
            end
        end
    end
