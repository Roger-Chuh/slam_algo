function [HH, camPoseVec,metricList1, ptUndist, intrMatHomo,pixList] = calibHomo2(inputDir, paraDir)
...
try
    if ~exist(fullfile(inputDir,'homo.mat'))
        ...
        metricList(:,end) = 1;
        figure,subplot(1,2,1);plot(pixList(:,1),pixList(:,2),'-r');hold on;plot(pixList(1,1),pixList(1,2),'*b');axis equal;subplot(1,2,2);plot3(metricList(:,1),metricList(:,2),metricList(:,3),'-r');axis equal;hold on;plot3(metricList(1,1),metricList(1,2),metricList(1,3),'*b');axis equal
