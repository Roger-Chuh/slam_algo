function ima_read_calib(calib_data,inputDir)
...
[~,image_numbers,type_numbering,N_slots] = check_directory(calib_data, inputDir);
...
while (i <= calib_data.n_ima), % & (~no_image_file),
   ...
   if calib_data.active_images(i),
      ...
      ima_name = [inputDir '\' calib_data.calib_name  number_ext '.' calib_data.format_image];
      ...
      if exist(ima_name),
         ...
         if 0%calib_data.format_image(1) == 'p',
            ...
            if 0 %calib_data.format_image(1) == 'r',
               Ii = readras(ima_name);
            else
