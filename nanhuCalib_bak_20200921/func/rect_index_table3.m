function [Irec,ind_new,ind_1,ind_2,ind_3,ind_4,a1,a2,a3,a4,imgL1] = rect_index_table3(I,R,f,c,k,alpha,KK_new,imgL)
...
if reverseMapping == 0
    pixOrig2 = remapRect([xuu2(:) yuu2(:)]', KK_new, Kinit,k, R);
else
    pixOrig2 = Orig2Rect([xuu2(:) yuu2(:)], Kinit,KK_new, R, k);
end
xOrigMat = reshape(pixOrig2(:,1),size(xuu2));
yOrigMat = reshape(pixOrig2(:,2),size(xuu2));

figure,imshow(zeros(nr,nc));hold on;plot(xuu2(:),yuu2(:),'.r');plot(pixOrig2(:,1),pixOrig2(:,2),'.g');
...
plotRow1 = 0;
for i = 1 : length(ySampled)
    
% %     pixRect_ = Orig2Rect([xuuAllUse(ySampled(i),:); yuuAllUse(ySampled(i),:)]', Kinit, KK_new, R,k);
