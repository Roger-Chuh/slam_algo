...
function calib_data = bundleAdjustmentUrban(calib_data, robust)
...
if 0
    ...
    calib_data.statEO{1,1}.sg0 = sqrt((v'*P0*v) / (rows-cols));
    % empirical covariance matrix
    Exx = pinv(J'*P0*J);
    % ExxEO = inv(Jext'*Jext);
    % ExxIO = inv(Jito'*Jito);
    calib_data.statEO{1,1}.Exx = Exx(offset:end,offset:end);
    calib_data.statEO{1,1}.varEO = diag(calib_data.statEO{1,1}.Exx);        % variance of ext ori parameters
    % standard deviation of ext ori parameters
    calib_data.statEO{1,1}.stdEO = calib_data.statEO{1,1}.sg0 * ...
        sqrt(abs(diag(calib_data.statEO{1,1}.Exx)));
    
    calib_data.statIO{1,1}.Exx = Exx(1:offset-1,1:offset-1);
    calib_data.statIO{1,1}.varIO = diag(calib_data.statIO{1,1}.Exx);
    % standard deviation of int ori parameters
    calib_data.statIO{1,1}.stdIO = calib_data.statEO{1,1}.sg0 * ...
        sqrt(abs(diag(calib_data.statIO{1,1}.Exx)));
end
%% rms
...
try
    ...
    figure(13);
    set(13,'Name','Calibration results after optimization','NumberTitle','off');
    subplot(2,1,1);
