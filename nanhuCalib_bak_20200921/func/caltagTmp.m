function [immm, box] = caltagTmp(I,datafile,debug)
% function [wPt,iPt] = caltagTmp(I,datafile,debug)
...
mode = 1;2;3;1;2; 3; 2; 3; 2; 1;3; 2;3; 2; 3;2;3;2; % [brad 10 Dec 2010] change from default 2 to 3, for alex's dataset
...
if max( [factor(imgHeight),factor(imgWidth)] ) > 1000000   %  21  roger 2018/06/08
...
if layout == 1
    switch mode
        ...
        case 2
            ...
            E = edge( I, 'sobel', 0.01, 'nothinning' );
%             E = edge( I, 'sobel', 0.1, 'nothinning' );
%             E = edge(I,'canny',[0.1,0.5]);
%             Se = strel('rectangle', [2,2]);
%             E = imerode(imdilate(E,Se),Se);
...
Bb = imerode(imdilate(Aa,SE1),SE2);
[Lb, numb] = bwlabel(Bb,8);
aa = uint8(Lb);
% region = [1 : num];
% for t = 1 : num
%     Len(t,:) = sum(L(:) == t);
...
cind = cind(cind~=0);
% % aa = labelmatrix(CC);
% SE = strel('rectangle', [50,55]);
SE = strel('rectangle', [10,10]);
if debug
    figure(10),clf,subplot(1,2,1);imshow(aa>0);subplot(1,2,2);imshow(zeros(size(I)),[]);hold on;
end
for uu = 1 : length(cind)
   s(uu,1) = length(find(aa(:) == cind(uu))); 
   imTmp = zeros(size(I));
   imTmp(find(aa(:) == cind(uu))) = 1;
   if 1
       im = imerode(imdilate(imTmp,SE),SE);
   else
       im = imTmp;
   end
   [y,x] = ind2sub(size(I),find(im(:) == 1));
   percnt(uu,1) = sum(imTmp(:) == 1)/sum(im(:) == 1);
   meanPos(uu,:) = [mean(x) mean(y)];
   if debug
       figure(10),subplot(1,2,2);plot(x,y,'.r');plot(mean(x),mean(y),'*g');drawnow;
   end
end
Y=pdist(meanPos);
Z=squareform(Y); %Z=linkage(Y);
aaa = Z<100&Z~=0;
% % % aaa = Z<150&Z~=0;
bb = tril(aaa);
[L, num] = bwlabel(bb,8);
...
bigReg = region(idx);
ind = find(L(:) == bigReg);
[yy,xx] = ind2sub(size(L),ind);
validLab = unique([xx;yy]);

imm = zeros(size(aa));
for ii = 1 : length(validLab)
   imm(aa == validLab(ii)) = 1;
end

index = find(imm(:) == 1);
[yyy,xxx] = ind2sub(size(aa),index);
% box = [[max(min(xxx)-100,1) max(min(yyy)-100,1)];[min(max(xxx)+100,size(aa,2)) min(max(yyy)+100,size(aa,1))]];
box = [[max(min(xxx)-margin,1) max(min(yyy)-margin,1)];[min(max(xxx)+margin,size(aa,2)) min(max(yyy)+margin,size(aa,1))]];
immm = zeros(size(aa));
immm(box(1,2):box(2,2),box(1,1):box(2,1)) = 1;
...
elseif 1 % small resolution
    [saddles] = cornerfinder_saddle_point( flipud(saddles_0), I, 13,13 ); %for sharp images: 5x5
    [saddles,good] = cornerfinder_saddle_point( saddles, I, 11,11 ); %for sharp images: 3x3
elseif 1 % very large resolution
    [saddles] = cornerfinder_saddle_point( flipud(saddles_0), I, 75,75 ); %for sharp images: 5x5
    [saddles,good] = cornerfinder_saddle_point( saddles, I, 63,63 ); %for sharp images: 3x3
else
    [saddles] = cornerfinder_saddle_point( flipud(saddles_0), I, 85,85 ); %for sharp images: 5x5
    [saddles,good] = cornerfinder_saddle_point( saddles, I, 43, 43 );
end
...
bottomleftorigin = false; true;
if bottomleftorigin
    iPt(:,1) = size(I,1) - iPt(:,1);
end
iPtTmp = iPt;
ipt = cornerfinder([iPtTmp(:,2),iPtTmp(:,1)]',double(II),5,5);
if debug
    figure,imshow(I);hold on;plot(iPtTmp(:,2), iPtTmp(:,1), 'or'); plot(ipt(1,:), ipt(2,:), 'xg');
end
