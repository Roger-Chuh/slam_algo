function [vrPath, vrPoseInfo,positiveFlag] = PlotRobotOnly(inputDir,startt)   %,timeInd)  %,posestPoseMapped)
...
for  i = startt : length(dirInfo)
    ...
    countSlam = 0;
    countRobo = 0;
    cntVR = []; cntRobo = []; accumAngVr = 0; vrPoseMat = [];
    if draw == 1
    ...
    while ~feof(pathFid)
        ...
        if flagg == 22     % 1
            ...
            try
                ...
                rmatVr = [[vrpath(7) 0 -vrpath(5)]' [0 1 0]' vrpath(5:7)']';
                vrPoseMat = [vrPoseMat;[reshape(rmatVr,1,9) vrpath(2:4)]];
                
                %             camPoseMat = [camPoseMat;[reshape(R,1,9) t]];
                if size(vrPoseMat,1) > 1
                    dltTVr =  [rmatVr vrpath(2:4)';0 0 0 1]*inv([reshape(vrPoseMat(end-1,1:9),3,3) vrPoseMat(end-1,10:12)';0 0 0 1]);
                    dltrVr = dltTVr(1:3,1:3);
                    dlttVr = dltTVr(1:3,end);
                    prvPtVr = [vrPoseMat(end-1,10);0;vrPoseMat(end-1,12)];
                    curPtVr = [vrPoseMat(end,10);0;vrPoseMat(end,12)];
                    dltAngVr = rad2deg(norm(rodrigues(dltrVr)));
                    errVr = dltrVr*prvPtVr + dlttVr - curPtVr;
                    if norm(rodrigues(roty(dltAngVr)) - rodrigues(dltrVr)) < norm(rodrigues(roty(-dltAngVr)) - rodrigues(dltrVr))
                        accumAngVr = [accumAngVr; accumAngVr(end) + dltAngVr];
                    else
                        accumAngVr = [accumAngVr; accumAngVr(end) - dltAngVr];
                    end
    ...
    if 0  % 20181214
        if accumAngVr(end) < 0
            accumAngVr = -accumAngVr;
            positiveFlag = 1;
        end
    else
        accumAngVr = accumAngVr;
    end
    vrPoseInfo = [vrPoseMat timeStamp2./1000 accumAngVr];
    abgf = 1;
