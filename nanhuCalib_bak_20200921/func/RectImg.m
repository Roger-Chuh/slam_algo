...
%         flag1 = pixRect_1(:,1) >=max(crop1(1),2) & pixRect_1(:,1)<=crop2(1) & pixRect_1(:,2) >=max(2,crop1(2)) & pixRect_1(:,2) <= crop2(2);
    ...
    if 0
        idInScale = find(mod(pixRect_1(:,2),scale) == 0);
        pixRect_1 = pixRect_1(idInScale,:);
    end
...
%     imgB = image(:,:,3);
    ...
    if ~isempty(pixRect_1)
        ...
        if 0
            ...
            FlagTmp = {};
            %             for j =  startPixRect(1) : scale: nc  % 1 : nc % endPixRect(1)
            for j =  startPixRect(1) : scale: crop2(1)  % 1 : nc % endPixRect(1)
                crop2
                if j == 110
                    sdku = 1;
                end
                ...
                if ~isempty(flagTmp)
                    ...
                    if ismember(2,flagTmp) && ~ismember(1,flagTmp) && ~isempty(pixHoldup)
                        pixWrite = [pixWrite;pixHoldup];
                        %                     StoreData = [StoreData; [mean(pixHoldup,1) size(pixHoldup,1)]];
                        StoreData = [StoreData; [pixHoldup repmat(size(pixHoldup,1),size(pixHoldup,1),1)]];
                        
                        if draw
                            figure(333),plot(pixHoldup(:,1),pixHoldup(:,2),'.','Color',rand(1,3));
                            foundInd = sub2ind([nr,nc],pixHoldup(:,2),pixHoldup(:,1));
                    ...
                    if ismember(2,flagTmp) && ~ismember(3,flagTmp) && ~isempty(pixHolddown)
                        pixWrite = [pixWrite;pixHolddown];
                        StoreData = [StoreData; [pixHolddown repmat(size(pixHolddown,1),size(pixHolddown,1),1)]];
                        
                        
                        if draw
                            figure(333),plot(pixHolddown(:,1),pixHolddown(:,2),'.','Color',rand(1,3));
                            foundInd = sub2ind([nr,nc],pixHolddown(:,2),pixHolddown(:,1));
                            xyOrig = round([xRect2Orig(foundInd) yRect2Orig(foundInd)]);
                            idIn = xyOrig(:,1) > 0 & xyOrig(:,1) <= nc & xyOrig(:,2) > 0 & xyOrig(:,2) <= nr;
                            foundIndIn = foundInd(idIn);
                            origInd = sub2ind([nr,nc],xyOrig(idIn,2),xyOrig(idIn,1));
