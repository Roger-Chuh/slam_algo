...
%   USA
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function click_calib(calib_data,config)
...
if manual_squares,
    ...
    calib_data.n_sq_x = config.Col - 1; %input(['Number of squares along the X direction ([]=' num2str(n_sq_x_default) ') = ']); %6
    if isempty(calib_data.n_sq_x), calib_data.n_sq_x = n_sq_x_default; end;
    calib_data.n_sq_y = config.Row - 1; %input(['Number of squares along the Y direction ([]=' num2str(n_sq_y_default) ') = ']); %6
...
if (isempty(calib_data.dX))|(isempty(calib_data.dY)), % This question is now asked only once
    ...
    calib_data.dX = config.dX; %input(['Size dX of each square along the X direction ([]=' num2str(dX_default) 'mm) = ']);
    calib_data.dY = config.dY; %input(['Size dY of each square along the Y direction ([]=' num2str(dY_default) 'mm) = ']);
...
%         [callBack, x, y]  = get_checkerboard_corners(kk,use_corner_find,calib_data,config.func);
        callBack = 1;
        x = rand(calib_data.n_sq_x+1,calib_data.n_sq_y+1);
        y = rand(calib_data.n_sq_x+1,calib_data.n_sq_y+1);
        if callBack == 1
            count = count + 1;
%             try
