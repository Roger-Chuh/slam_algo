function [disp,accumAng,ind0,pt,data,timeStamp] = readBeiDumpCalib2(inputDir,paraDir,ang00, dist00,useLR)
...
% %             movefile('D:\ForDump\apriltag\bin\test.mp4', inputDir);
            movefile('D:\ForDump\apriltag\cmake-build-debug2\test_003.mkv', inputDir);
        catch
            asgjokhk = 1;
        end
        try
...
%     0.079000004*2, 0, 0;
%     0, 0, 0];
height = 165; 190; 165;125;  135;125;135; 160; 135; 0;125;135;  0; 135;125;132;100; 129;
...
if forTest == 0
    ...
    try
        load(fullfile(paraDir, 'heightOpt.mat'));
        heightOpt = calcHeight;
        calcHeight = [];
        load(fullfile(paraDir, 'offset.mat'));
...
cbSize = 170.3511;     170.9539-1; 170.4792;  156.6186;   158; 156.6186; 158;158; 170.4792;
...
[H, camPoseVec, metricList1, ptUndistHomo, intrMatHomo] = calibHomo2('D:\Temp\20181026\calibFishEyeHomo1', 'E:\bk_20180627\pc\Temp\20180724\calibFishEye1');    
...
try
    ...
    mini = mini1(mini2);
% %     [indexCellRobo_, indexRobo_] = splitIndex2(find(abs(diff(roboPoseInfo(end:-1:1,14))./diff(roboPoseInfo(end:-1:1,13))) < 1)); % 0.2
% %     tmp_ = medfilt1(abs(diff(roboPoseInfo(end:-1:1,14))./diff(roboPoseInfo(end:-1:1,13))),2*window3-1); %diff(roboPoseInfo(:,14))./diff(roboPoseInfo(:,13));
% %     [minv_,mini1_]=findpeaks(-tmp_,'minpeakdistance',1);
% %     mini2_ = find(tmp_(mini1_) < 2);
% %     mini_ = mini1_(mini2_);
% %     minii = find(ismember(roboPoseInfo(:,end),roboPoseInfo(mini_,end)));
...
dltAngRobo = abs(diff(roboPoseInfo(Mini,end)));
...
figure(21),hold on;plot([dltAngRobo(1);dltAngRobo;dltAngRobo(end)])
% % figure,
...
try
    ...
    for i = 1 : length(indexCell0)-1
        ...
        iddd = [iddd;round(mean(indexCell0{i}));round(mean(indexCell0{i+1}))];
    end
    iddd = unique(iddd);
    mean(accumAngMat)
    figure(21),plot(accumAngMat);
    figure,subplot(1,2,1);hist(accumAngMat(1:size(accumAngMat,1)/2));subplot(1,2,2);hist(accumAngMat(size(accumAngMat,1)/2 +1:end));
    ...
    figure,plot(timeStamp,accumAng);hold on;
    plot(timeStamp(iddd),accumAng(iddd),'*r');
    plot(timeStamp(ind0),accumAng(ind0),'*g');plot(roboPoseInfo1(:,13),roboPoseInfo1(:,14));
    for u = 1 : length(accumAngMat)
        text(timeStamp(iddd(u),:)+0.1,accumAng(iddd(u),:)+30,num2str(round(accumAngMat(u),2)), 'Color',[1 0 0],'FontSize',15);  %,'FontWeight','bold');
    end
    plot(roboPoseInfo(:,end-1),roboPoseInfo(:,end));hold on;plot(roboPoseInfo(Mini,end-1),roboPoseInfo(Mini,end),'*m')
    
    for u = 1 : length(dltAngRobo)
        text(roboPoseInfo(Mini(u),end-1)+0.2,roboPoseInfo(Mini(u),end)+30,num2str(round(dltAngRobo(u),2)), 'Color',[0 0 1],'FontSize',15);  %,'FontWeight','bold');
    end
    plot(slamPoseInfo1(:,13),slamPoseInfo1(:,14));
