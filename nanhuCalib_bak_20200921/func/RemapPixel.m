function [xRectMatUse, yRectMatUse,KK_new, Kinit,k,imgL1, LutVecCharX, LutVecCharY, SplitIndX, SplitIndY,lutVecX,lutVecY] = RemapPixel(I,R,f,c,k,alpha,KK_new,imgL,stepXY,reverseMapping,varargin)
...
%     [xRectMat1, yRectMat1, LutVecCharX, LutVecCharY, SplitIndX, SplitIndY,lutVecX,lutVecY] = BiLinearInterp(inputDir,xOrigMat,yOrigMat,xRectMatRef,yRectMatRef,scaleSizeX,scaleSizeY,nc,nr,doRounding,optFrac); %bilinear2x2Func
    [xRectMat1, yRectMat1, LutVecCharX, LutVecCharY, SplitIndX, SplitIndY,lutVecX,lutVecY] = BiLinearInterp_(inputDir,xuu2, yuu2, xOrigMat,yOrigMat,xRectMatRef,yRectMatRef,scaleSizeX,scaleSizeY,nc,nr,doRounding,optFrac); %bilinear2x2Func
else
...
function [xRectMat, yRectMat, LutVecCharX, LutVecCharY,SplitIndX,SplitIndY,lutVecX,lutVecY] = BiLinearInterp_(inputDir,xuu2, yuu2, xOrigMat,yOrigMat,xRectMatRef,yRectMatRef,scaleSizeX,scaleSizeY,nc,nr,doRounding,optFrac)
...
[xRectMat,LutVecCharX,SplitIndX,lutVecX] = bilinear2x2_(xuu2, yuu2, xOrigMat,scaleSizeX,scaleSizeY,nc,nr,intLen,fixFrac,optFracCell,doRounding,optFrac);
[yRectMat,LutVecCharY,SplitIndY,lutVecY] = bilinear2x2_(xuu2, yuu2, yOrigMat,scaleSizeX,scaleSizeY,nc,nr,intLen,fixFrac,optFracCell,doRounding,optFrac);
[~,errr] = NormalizeVector([xRectMat(:) yRectMat(:)] - [xRectMatRef(:) yRectMatRef(:)]);
...
function [b, LutVecChar, SplitInd,LutVec] = bilinear2x2_(xuu2, yuu2, AA, kx,ky,nc,nr,intLen,fixFrac,optFracCell,doRounding,optFrac)
SplitInd = [];  LutVec = [];
optFrac = [optFrac(1:3) max(optFrac(2:3)) optFrac(4:end)];
tmp = cell2mat(optFracCell')';
fixFracId = setdiff([1:length(intLen)],cell2mat(optFracCell')')';
...
%             storeXY{cnt,4} = Num2Fix_(dot(repmat(storeXY{cnt,2},length(id),1)',storeXY{cnt,3}')',14,fracVec(3));
            storeXY{cnt,4} = Num2Fix(Num2Fix(repmat(storeXY{cnt,2}(1),length(id),1).*storeXY{cnt,3}(:,1),14,fracVec(3))...
                ...
                + Num2Fix(repmat(storeXY{cnt,2}(4),length(id),1).*storeXY{cnt,3}(:,4),14,fracVec(3)),14,fracVec(4));
            storeXY{cnt,5} = id;
            cnt = cnt + 1;
        end
    end
