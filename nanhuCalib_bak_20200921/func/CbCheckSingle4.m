function [goodId, cbcXY] = CbCheckSingle4(cbImgList,calibFuncDir,modeE,blbl, varargin)
...
if modeE == 1
  calib_data.n_sq_x = 8;  %% vert
  calib_data.n_sq_y = 13;
else

calib_data.n_sq_x = 13; %% hori
calib_data.n_sq_y = 8;
end
...
%%   
    try
%         a;
    [callBack, initCbcX, initCbcY]  = get_checkerboard_corners5(iImg,0,calib_data,calibFuncDir);
    catch
        try
% % %             [initCbcX, initCbcY] = DetectCbCorner(cbImg);
            tttttt;
...
%         camParam = [];
        ...
        noGoodId = [noGoodId; iImg];
        fprintf('\n\n===== image %s... not all corners are detected =====\n\n',cbImgList{iImg}(end-15:end));
% % % % %         fprintf('\n\n===== not all corners are detected ======\n\n');
...
% %     good = 0;
    [isbl, bl] = isBlur(imread(cbImgList{iImg}),blbl);
...
% %         camParam = [];
        noGoodId = [noGoodId; iImg];
% % %         fprintf('\n\n===== corners are not arranged properly ======\n\n');
        fprintf('\n\n===== image %s... corners are not arranged properly =====\n\n',cbImgList{iImg}(end-15:end));
...
if ~isempty(noGoodId)
    goodId = setdiff([1:nImg]',noGoodId);
else
    goodId = [1:nImg]';
end
...
while (change > 1e-9 && iter < maxIter)
    ...
    focCurr = param(1:2);
    cenCurr = param(3:4);
    if (centerOptim && (cenCurr(1) < 0 || cenCurr(1) > imgW || cenCurr(2) < 0 || cenCurr(2) > imgH))
