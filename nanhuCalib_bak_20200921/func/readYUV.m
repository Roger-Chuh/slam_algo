function readYUV( inputDir ,width, height,scale)
...
for i =    1 : frameNum  %[1 2 4 6 8 13 14 16 18 20 25 26 28 30 35 36 37 39 41 47 48 49 51 53 55 58 60 61 63 65 67]
    ...
    elseif 1
        CC = imgaussfilt(uint8(CC), 1.2);
        DD = imgaussfilt(uint8(DD), 1.2);
    else
        sadghk = 1;
    end
    if 0
...
%         drawnow;
    ...
    asrgkj = 1;
    if  1 % mod(i,2) == 0
        imwrite(imresize(uint8(CC),[height/scale(1),width/scale(2)]), fullfile(inputDir,sprintf('imgL_%05d.png',i)));
