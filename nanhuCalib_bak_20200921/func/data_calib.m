...
function data_calib(calib_data,config)
...
while (Nima_valid==0),
   ...
   fprintf(1,'\n');
   calib_data.calib_name = config.baseName; % input('Basename camera calibration images (without number nor suffix): ','s');
   
   calib_data.format_image = '0';
   calib_data.format_image = config.suffix;
...
%    end;
   ...
   [Nima_valid] = check_directory(calib_data, config.inputDir);
...
if (Nima_valid~=0),
    ...
    ima_read_calib(calib_data,config.inputDir); % may be launched from the toolbox itself
