function [T, roboMap, vrMap, TClient, roboPosePoseMatStamp, slamPosePoseMatStamp] = PlotRobotVR2(inputDir,startt,figNum)
dirInfo = dir(fullfile(inputDir,'comm_client_log*.txt'));
% close all
% TGood = 0;
% % figNum = 80;
% % % figure(3),clf;
...
for  i = startt : length(dirInfo)
    ...
    if accumAngRobo(end) < 0
        accumAngRobo = -accumAngRobo;
    end
    
    if accumAngSlam(end) < 0
